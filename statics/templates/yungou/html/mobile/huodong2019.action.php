 ...
 //        }
	...
	*/
	...
	public function activity_time(){
		...
		if($user){
			...
			$flag = $this->db->GetOne("SELECT * FROM `@#_wxpay_locat` WHERE `uid` = '$uid' AND `status` = '1' AND `create_time`>'$time' order by `create_time` desc");
	   		...
	   		$arr3 = $this->db->GetCount("SELECT COUNT(*) FROM `@#_activity_yyy` WHERE `user_id` = '$uid' AND `current_count` LIKE '%$tt%'");
	   		if($arr3 >= 1){
		...
		if($data){
			...
			if (!empty($data2['bind_phone'])) {
				if (strpos($data2['bind_phone'],$data1['mobile']) !== false) {
					...
					$this->db->Query("UPDATE `@#_activity_yyy` SET `company` = '惠享配送',`company_code` = '$company_code' WHERE `id` = $sid");
				}
			}
			if ($q) {
				echo 1;
	...
	public function swapOrderDetail(){
		...
		$flag = 'huodong';
		$str = $this->db->GetOne("SELECT m.*,n.logo,n.name FROM `@#_activity_yyy` AS m LEFT JOIN `@#_zhuanpan_shop` AS n ON m.amount = n.id WHERE m.id = '$id' AND m.user_id = '$uid'");
		$str['thumb'] = $str['logo'];
		$str['title'] = $str['name'];
