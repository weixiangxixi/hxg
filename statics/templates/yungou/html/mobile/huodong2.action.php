...
class huodong2 extends memberbase {
	...
	public function recharge111(){
		...
		if($user){
			...
			$flag2 = $this->db->GetOne("SELECT * FROM `@#_wxpay_locat` WHERE `uid` = '$user_id' AND `status` = '1' order by `create_time` desc");
			$tt = date('Ymd',time());
			$arr3 = $this->db->GetCount("SELECT COUNT(*) FROM `@#_activity_lottery` WHERE `user_id` = '$user_id' AND `current_count`='$tt'");
			if(!$arr3){
				$arr3 = 0;
			}
			$sycs = 2-$arr3;
        ...
        $current_count = intval(date("Ymd",time()));
        $arr = $this->db->GetList("SELECT m.*,n.name FROM `@#_activity_lottery` AS m LEFT JOIN `@#_zhuanpan_shop` AS n ON m.amount = n.id WHERE m.current_count='$current_count' order by m.id desc");
	...
	*/
	...
	public function inrecharge(){
		...
		$str = $this->db->GetOne("SELECT * FROM `@#_zhuanpan_shop` WHERE `id` = '$s'");
		...
		if($xxx <= 2){
			...
			if($s >= 1 && $s <= 4){
				$flag = $this->db->Query("INSERT INTO `@#_member_account`(`uid`,`type`,`pay`,`content`,`money`,`time`) VALUE ('$uid','1','积分','摇一摇奖励','$str[flag]','$time')");
				...
				if($x && $flag && $f){
					...
					if(!$hd_flag && $hd_data['user_id'] != $uid && $hd_sid){
						...
						$hd_status = $this->db->Query("UPDATE `@#_activity_lottery` SET `friends_id` = '$friends_id' WHERE `id` = '$hd_sid'");
						if($hd_status){
							$data['hd'] = 1;
						}else{
							$data['hd'] = 2;
