$(function() {
    ...
    var b = function() {
        ...
        $('#dyq_sel i').click(function(){
            if($(this).attr('class') == 'z-pay-ment-no'){
                return false;
            }
            if($(this).attr('class') == 'z-pay-ment'){
                $(this).attr('class','z-pay-mentsel');
            }else if($(this).attr('class') == 'z-pay-mentsel'){
                $(this).attr('class','z-pay-ment');
            }
            $(this).parent().siblings().children('i.z-pay-mentsel').attr('class','z-pay-ment');

            var dyq = $('#dyq_sel .z-pay-mentsel').data('val');
            dyq_id = $('#dyq_sel .z-pay-mentsel').data('id');
            if(dyq_id == null){
                dyq_id = 0;
            }
            if(dyq != null && x >= 20){
                if(dyq + y >= x){
                    ...
                    $('#spBalance').parent().addClass("z-pay-grayC");
                    $('#spBalance').parent().css('color','#ccc');
                    $("#bankList li").children().attr('class','z-pay-mentsel');
                    $("#bankList li").children().attr('sel','1');
                }
            }else{
                var index = $("#bankList li").index();
                var z = $("#bankList li").children().attr('class');
                if(index != 0){
                    if(z == 'z-pay-ment'){
                        if(x < 20){
                            $('<a href="/index.php/mobile/mobile/glist" style="height: 45px;line-height: 45px;" id="btnPay" class="orgBtn noMoney">满20使用微信支付,前往选购</a>').replaceAll("#btnPay");
                        ...
                        $("#bankList li").children().attr('class','z-pay-ment');
                        $("#bankList li").children().attr('sel','0');
                        if(parseInt(d) < x && x >= 20){
                            $('<a style="height: 45px;line-height: 45px;" id="btnPay" href="javascript:;" class="orgBtn">微信支付</a>').replaceAll("#btnPay");
                        }else if(parseInt(d) < x && x < 20){
            ...
            if (w + g >= x) {
			    ...
			    if (shopnum != -1) {
					...
					if (shopnum == 0) {
                        if(wx == 1){
                            ...
                            location.replace(Gobal.Webpath+"/mobile/cart/paysubmit_test/money?dyq="+dyq_id)
                        }else{

                            location.replace(Gobal.Webpath+"/mobile/cart/paysubmit_test/"+checkpay+"/"+banktype+"/"+x+"/"+t+"/"+submitcode+"?dyq="+dyq_id)
                        }
                ...
                if (e > 0) {
                    ...
                    if (v == 1 || v == 2 || v == 3) {
                           ...
                           location.href = Gobal.Webpath+"/mobile/cart/paysubmit_test/"+checkpay+"/"+banktype+"/"+x+"/"+t+"?dyq="+dyq_id
