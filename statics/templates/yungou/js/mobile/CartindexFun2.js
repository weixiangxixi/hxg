...
$(".tags li").on("click",function(){
 ...
 $(function() {
    var ssss = $("input:text[name=num]").val();
    var yyyy = $("#hidBalance").val();
    //alert(ssss);
    if ((ssss < 9 || ssss == 9) && yyyy < 1 ) {
        $('.orgBtn1').removeAttr("onclick"); 
        $('.orgBtn1').css('background-color','#c8d0d5');
    }
    if ((ssss == 10 || ssss > 10) && yyyy < 1 ) {
        $('.orgBtn1').attr("onclick","turn_url();");
        $('.orgBtn1').css('background-color','#22AAFF');
    }
 });
     ...
     var b = function() {
         ...
         var m = function() {
             ...
             if (r > 0) {
                 ...
                 g.children("p").html('将获取以上活动商品<span class="orange arial z-user">' + q + '</span>个参与号码  合计金额：<span id="total" class="orange arial">' + q + ".00</span> 元")
                 //alert(q);
                 $('.wechat').text(q + ".00");
                 var yyyy = $("#hidBalance").val();
                 if ((q<9 || q==9) && yyyy < 1) {
