    ...
    var b = function() {
        ...
        var q = function(y) {
            ...
            if (y > 0) {
                ...
                c.attr("sel", "1").attr("class", "z-pay-mentsel sle").next("span").html('参与币支付<em class="orange">' + y + ".00</em>元（您的参与币：" + t + "）")
				$("#payact").val("zh");
                checkpay='fufen';
				banktype='nobank';
            } else {
                c.attr("sel", "0").attr("class", "z-pay-ment sle").next("span").html('参与币支付<em class="orange">0.00</em>元（您的参与币：' + t + "）")
            }
        };
        var f = function(y) {
            ...
            if (y > 0) {
                ...
                p.attr("sel", "1").attr("class", "z-pay-mentsel sle").next("span").html('余额支付<em class="orange">' + y + ".00</em>元（账户余额：" + d + " 元）")
				$("#payact").val("zh");
                checkpay='money';
				banktype='nobank';
            } else {
                p.attr("sel", "0").attr("class", "z-pay-ment sle").next("span").html('余额支付<em class="orange">0.00</em>元（账户余额：' + d + " 元）")
            }
        };
        var k = function(y) {
            return false;
        ...
        if (parseInt(d) > 0) {
            ...
            p1.parent().click(function() {
                if(p1.hasClass('z-pay-mentsel')){
                    $("#shop_score").val(0);
                    p1.removeClass('z-pay-mentsel').addClass('z-pay-ment').attr("sel",'1');
                }else{
                    $("#shop_score").val(1);
                    p1.removeClass('z-pay-ment').addClass('z-pay-mentsel').attr("sel",'1');
                }

            });
            p.parent().click(function() {
                if(p.hasClass('z-pay-mentsel')){
                    $("#moneycheckbox").val(1);
                }else{
                    $("#moneycheckbox").val(0);
                }
