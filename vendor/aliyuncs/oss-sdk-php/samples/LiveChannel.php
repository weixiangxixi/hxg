...
/**
    创建一个直播频道
    频道的名称是test_rtmp_live。直播生成的xc文件叫做test.xc，该索引文件包含3片ts文件，每片ts文件的时长为5秒（这只是一个建议值，具体的时长取决于关键帧）。
 */
...
/**
    创建直播频道之后拿到推流用的play_url（rtmp推流的url，如果Bucket不是公共读写权限那么还需要带上签名，见下文示例）和推流用的publish_url（推流产生的xc文件的url）
 */
 ...
 *  播放列表指定为“vod_playlist.xc”，也就是说这个接口调用成功之后会在OSS上生成一个名叫“vod_playlist.xc”的播放列表文件。
 */
$current_time = time();
