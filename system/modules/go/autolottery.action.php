...
class autolottery extends SystemAction {
	...
	public function autolottery_ret_install(){
		...
		if($u_go_info){
			...
			file_put_contents("/www/wwwroot/fuck.log","{$id}:{$q_uid} -- {$_SERVER['PHP_SELF']}\n",FILE_APPEND);
				sendWarnMsg($id, $q_uid);
      			$q_1 = true;
			...
			if($u_go_info){
					...
					file_put_contents("/www/wwwroot/mall/sql_log/autolottery_install_b.log","{$_SERVER['PHP_SELF']}:{$sql}\n",FILE_APPEND);
			}else{
