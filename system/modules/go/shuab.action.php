...
class shuab extends base {
	...
	public function __construct(){	
		...
		public function shua(){
			$data  = $this->db->GetOne("SELECT `cfg_value` FROM `@#_wxch_cfg` WHERE  `cfg_name` = 'auto2'  LIMIT 1");
			$data = unserialize($data['cfg_value']);
			$t1= $data['maxtime'];
			$t2= $data['mintime'];
			include templates("index","shaidan12345");
		}
		//购买商品
		public function buyshop($id,$shopinfo,$member){
			...
			if($shopinfo['zongrenshu'] > $shopinfo['canyurenshu']){
                ...
                $tmnp_ = json_encode($rs);
                //file_put_contents("/www/wwwroot/mall/sql_log/shuadan_pay_user_go_shop.log", "{$user_id}:{$shopid}:{$shopnum}:{$tmnp_}\n", FILE_APPEND);
				...
				if($autoadd == 1){
					...
					// 最后存储到数据库
					$temp = serialize($data);
					$rs = $this->db->Query("UPDATE `@#_wxch_cfg` SET `cfg_value` = '$temp' WHERE `cfg_name` = 'auto2'");
					if(!$rs){
						echo '进入下一期并失败！';exit;
					}
