...
class wxorder4 extends SystemAction {
	...
	public function init(){
		$url = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid='.$this->conf['weixin']['id'].'&redirect_uri='.WEB_PATH.'/api/wxorder4/callback&response_type=code&scope=snsapi_userinfo&state=wechat123&connect_redirect=1#wechat_redirect';
		header("location:$url");
	}
	//wexin回调
	public function callback(){
  	...
  	public function save_info(){
		...
		//$qq_openid = $this->qq_openid;
      	$str = '2';
		$rs = $this->db->Query("UPDATE `@#_member` SET `wxopenid1` = 'a' WHERE `mobile` = '13533064216'");
      	if($rs){
			return true;
        }else{
        	return false;
  	...
  	public function ab(){
    	$go_user_info = $this->db->GetOne("select * from `@#_member` where `mobile` = '13533064216' LIMIT 1");
      	var_dump($go_user_info);
    }
	//wexin回调
