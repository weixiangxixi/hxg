...
class xypay_web_url extends SystemAction {
    ...
    public function houtai(){
  	...
  	public function houtai3(){
        ...
        $sign = strtoupper(md5($md5str . "key=" . $md5key));
      	...
      	//file_put_contents("/www/wwwroot/csthsc/sql_log/pay_insert_shop.log", json_encode($_REQUEST)."\n", FILE_APPEND);
		...
		$up_q1 = $this->db->Query("UPDATE `@#_member_addmoney_record` SET `wx` = '2', `pay_type` = '微信公众号', `status` = '已付款' where `id` = '$dingdaninfo[id]' and `code` = '$dingdaninfo[code]'");
		$up_q2 = $this->db->Query("UPDATE `@#_member` SET `money` = `money` + $total_fee_t where (`uid` = '$dingdaninfo[uid]')");
		$up_q3 = $this->db->Query("INSERT INTO `@#_member_account` (`uid`, `type`, `pay`, `content`, `money`, `time`) VALUES ('$dingdaninfo[uid]', '1', '账户', '通过微信公众号充值', '$total_fee_t', '$time')");
		...
		$scookies = unserialize($dingdaninfo['scookies']);
        //file_put_contents("/www/wwwroot/csthsc/sql_log/pay_insert_shop.log", json_encode($dingdaninfo)."\n", FILE_APPEND);
		$pay = System::load_app_class('pay','pay');
		$pay->scookie = $scookies;
		$pay->fufen=$dingdaninfo['score'];
        ...
        echo "ok";$_SESSION["out_trade_no"] = '';$_SESSION["total_fee"] = '';exit;
                //exit();
            } 
        }else{
            echo "fail";exit;
  ...
  public function houtai444(){
	...
	public function houtai5(){
        ...
        if ($status == 'TRADE_SUCCESS') {
			...
			$up_q1 = $this->db->Query("UPDATE `@#_member_addmoney_record` SET `wx` = '1', `pay_type` = '微信公众号', `status` = '已付款' where `id` = '$dingdaninfo[id]' and `code` = '$dingdaninfo[code]'");
			$up_q2 = $this->db->Query("UPDATE `@#_member` SET `money` = `money` + $total_fee_t where (`uid` = '$dingdaninfo[uid]')");
			$up_q3 = $this->db->Query("INSERT INTO `@#_member_account` (`uid`, `type`, `pay`, `content`, `money`, `time`) VALUES ('$dingdaninfo[uid]', '1', '账户', '通过微信公众号充值', '$total_fee_t', '$time')");
  ...
  public function houtai55(){
	  ...
	  public function alipay(){
	      	...
	      	$real_fee = $_POST['real_fee'];
	      	$total_fee = $_POST['total_fee'];
	      	$sign = $_POST['sign'];
	      	
	      	$key = 'fe31d4328d274d579bc431cf2c0dba0c';

	      	$rs_sign = md5($mer_id . $out_trade_no . $pay_type . $real_fee . $total_fee . $key);

	      	file_put_contents("/www/wwwroot/csthsc/sql_log/alipay.log", json_encode($mer_id ."-". $out_trade_no ."-". $pay_type ."-". $real_fee ."-". $total_fee ."-". $key)."--{$rs_sign}\n", FILE_APPEND);
	      	echo 'success';exit;
	      	if($rs_sign == $sign){
	      		echo 'success';exit;
	      	}else{
