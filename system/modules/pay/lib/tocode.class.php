...
class tocode {
	...
	public function run_tocode(&$time='',$num=100,$cyrs='233',$opencode=0){
	...
	private function get_code_dabai(){
		...
		$count_time2 = $count_time + $this->opencode;
		$this->count_time2 = $count_time2;
		$this->go_content = serialize($html);
		$this->count_time=$count_time;
		$this->go_code = 10000001+fmod($count_time2,$this->cyrs);			
	}
	...
	private function get_code_yibai(){		
		...
		$w=substr($time,11,3);
		$num= $h.$i.$s.$w;
		$num2 = $num + $this->opencode;
		if(!$cyrs)$cyrs=1;
		$this->go_code = 10000001+fmod($num2*100,$cyrs);
	...
	private function get_code_user_html(){		
		$time = $this->times;
		$num  = $this->num;
		$this->go_list = $this->db->GetList("select * from `@#_member_go_record` where `time` <= '$time' order by `time` DESC limit 0,$num");
