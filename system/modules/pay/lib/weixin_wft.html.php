			...
			var onBridgeReady = function() {
				...
				function(res) {
					if (res.err_msg == "get_brand_wcpay_request:ok") {
						//location.replace("<?php echo WEB_PATH ?>/mobile/home/userbalance/1");
						<?php if($shopping) {?>
						  location.replace("<?php echo WEB_PATH ?>/mobile/cart/paysuccess");
						<?php }else{ ?>
						  location.replace("<?php echo WEB_PATH ?>/mobile/home/");
						<?php } ?>
					} else if (res.err_msg == "get_brand_wcpay_request:cancel") {
						location.replace("<?php echo WEB_PATH ?>/pay/weixin_wft_url/payinfo/cancel");
					} else if (res.err_msg == "get_brand_wcpay_request:fail") {
						location.replace("<?php echo WEB_PATH ?>/pay/weixin_wft_url/payinfo/fail");
					} else {
						location.replace("<?php echo WEB_PATH ?>/pay/weixin_wft_url/payinfo/" + encodeURIComponent(res.err_msg));
					}
				});
			};
