...
class xypay_web_url2 extends SystemAction {
  	...
  	public function yaoqianshu()
    {
        ...
        $xml=json_decode($xmljson,true);
      	...
      	$rs_sign = $xml['sign'];
      	$amount = $xml['amount'];
      	$fl_order_no = $xml['fl_order_no'];
      	$mch_id = $xml['mch_id'];
      	$status = $xml['status'];
      	$order_no = $xml['order_no'];
        
      	
		$key = '47325ea12e25ddegeff7d4003d6g0110';
      	
      	$str = 'amount='.$amount.'&fl_order_no='.$fl_order_no.'&mch_id='.$mch_id.'&order_no='.$order_no.'&status='.$status.'&key='.$key;
        ...
        if($xml['status'] == '100'){
          	...
          	//$rs = json_encode($data);
            //file_put_contents("/www/wwwroot/csthsc/sql_log/pay_insert_shop.log", "{$rs}\n", FILE_APPEND);
            $total_fee_t = $xml['amount'];
            $out_trade_no = $xml['order_no'];
			//file_put_contents("/www/wwwroot/csthsc/sql_log/pay_call.log", "{$total_fee_t}:{$out_trade_no}\n", FILE_APPEND);exit;
            $dingdaninfo = $this->db->GetOne("select * from `@#_member_addmoney_record` where `code` = '$out_trade_no'");
            if(!$dingdaninfo){
                echo "error";exit;
  	...
  	public function yaoqianshu1()
    {
		...
		$key = '0211d1b0e24e133c4ea6f620g1a3ga6b';
      	//$key = 'e70387ce2a07eded4ge2b34785a730a8';
