...
function pay_insert_shop($shop = '', $type = '')
{
    ...
    if($cate_id == '173' && ($time_flag < 6900 || $time_flag > 35700)){
        if($time_flag <= 6900){
            $times = ceil($time_flag/300);
            $str = strtotime(date("Y-m-d",time())) + $times*5*60;
        }else if($time_flag > 35700 && $time_flag <= 36000){
            $times = 24;
            $str = strtotime(date("Y-m-d",time())) + 10*60*60;
        }else if($time_flag > 36000 && $time_flag <= 79200){
            $times = ceil(($time_flag-36000)/600) + 24;
            $str = strtotime(date("Y-m-d",time())) + 10*60*60 + ($times-24)*10*60;
        }else if($time_flag > 79200){
            $times = ceil(($time_flag-79200)/300) + 96;
            $str = strtotime(date("Y-m-d",time())) + 22*60*60 + ($times-96)*5*60;
        }
        ...
        $query = true;
        $time_tmg = time();
        $q = $db->Query( "UPDATE `@#_shoplist` SET `canyurenshu`=`zongrenshu`,`shenyurenshu` = '0',`q_end_time` = '$str',`v` = '$time',`q_showtime` = '$q_showtime' where `id` = '$shop[id]'");
		...
		sendWatchMail("更新中奖1：中奖uid：".$u_info['uid']."  商品shopid：".$shop['id']);
    ...
    if ($query) {
		$file = "/www/wwwroot/m.666cf.cn/lock/{$shop['sid']}.lock";
        $fp = fopen($file , 'a+');    
        if(flock($fp , LOCK_EX)){    
        ...
        if ($shop['qishu'] < $shop['maxqishu']) {
            ...
            //file_put_contents("/www/wwwroot/m.xx.com/sql_log/pay_insert_shop.log", "{$shop['id']}:{$shop['zongrenshu']}:{$shop['canyurenshu']}\n", FILE_APPEND);
...
function tmg_pay_shop($shop = '',$opencode){
        ...
        if($ex_code[$ex_rand]){
            ...
            if(!empty($chazhi)){
                ...
                $db->Query("UPDATE `@#_member_go_record` SET `time`='$str_t_time' where `id` = '{$last_info['id']}'");
                    ...
                    $tocode->run_tocode($str_t_time,100,$shop['canyurenshu'],$opencode,$shop);
	...
	sendWatchMail("更新中奖2：中奖uid：".$u_info['uid']."  商品shopid：".$shop['id']);
        $sqlss = "UPDATE `@#_shoplist` SET
