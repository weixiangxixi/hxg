...
class xypay_web_url4 extends SystemAction {
  	...
  	public function wechat111()
    {
        ...
        foreach ($xml as $k => $v) {
      	...
      	$key = '604479E7058A43949E6DF479B541E29D';
      
      	$rs_sign = strtoupper(md5($req.$key));
      
      	$sign = $_SERVER['HTTP_X_QF_SIGN'];
      
      	file_put_contents("/www/wwwroot/csthsc/sql_log/paycall.log", $sign."-mysign:".$rs_sign."-".json_encode($xml)."\n", FILE_APPEND);
        ...
        if($xml['status'] == "1"){
			...
			$up_q1 = $this->db->Query("UPDATE `@#_member_addmoney_record` SET `wx` = '5', `pay_type` = '微信公众号', `status` = '已付款' where `id` = '$dingdaninfo[id]' and `code` = '$dingdaninfo[code]'");
