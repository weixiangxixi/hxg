...
class wxpay_web_url extends SystemAction {
    ...
    public function houtai(){
		...
		//其他关于充值的函数
        flow($total_fee_t,$uid);//购买送流量
        news_user_song($uid);//新注册送8.88
		...
		$c_flow = $total_fee_t * 0.1;
	  	...
	  	if (empty($c_if)) {
	  	  	$this->db->Query("INSERT INTO `@#_member_flow` (`uid`, `flow`) VALUES ('$uid', '$c_flow')");
	  	}else{
	  	  	$this->db->Query("UPDATE `@#_member_flow` SET `flow` = `flow` + $c_flow where (`uid` = '$uid')");	
	  	}
		...
		if ( $check ) {
			...
			//$this->db->Query("INSERT INTO `@#_member_activity_money` (`uid`, `money`, `time`, `status`) VALUES ('1', '2', '3', '4')");
			echo "1";exit;
		} else {
			echo "fail";exit;
		}

		$pay->init($uid,$pay_type['pay_id'],'go_record');	//参与商品
