...
class home extends base {
	public function __construct(){ 
		...
		if(!$this->userinfo['mobile'] && !strpos($_SERVER['PHP_SELF'], 'mobile') && !$this->isAjax()){
			header("location:".WEB_PATH."/member/home/mobilechecking/");exit;
		}
	...
	public function sendsuccess(){		
		...
		$content='
	<span>请在24小时内绑定邮箱</span>
	，点击链接：
	<a href="'.WEB_PATH.'/member/home/emailcheckingok/'.$strcode.'">
		';
		$content.=WEB_PATH.'/member/home/emailcheckingok/'.$strcode.'
	</a>
	';
	...
	<24){
			$this->
		db->Query("UPDATE `@#_member` SET email='".$memberx[0]."',emailcode='1' where uid='$member[uid]'");			
			$title="邮箱验证完成";
...
//福分、经验添加			
	...
	public function updateddress(){
		...
		if(isset($_POST['submit'])){
			...
			$jiedao1=isset($_POST['jiedao']) ? $_POST['jiedao'] : "";
			$jiedao = preg_replace( "@<script(.*?)</script>@is", "", $jiedao1 ); 
			...
			$time=time();
			if ($jiedao==null) {
				echo _message ("街道包含特殊字符",WEB_PATH."/member/home/address",3);
				exit;
			}
			if($sheng==null or $jiedao==null or $shouhuoren==null or $mobile==null){
				echo _message ("带星号不能为空",WEB_PATH."/member/home/address",3);
				exit;
			}			
			if(!_checkmobile($mobile)){
				echo _message ("手机号码错误",WEB_PATH."/member/home/address",3);
				exit;
			}
	...
	public function invitefriends(){
		...
		for($i=0;$i
		   ...
		   //查询邀请好友的消费明细		   
		   $accounts[$sqluid]=$mysql_model->
						GetList("select * from `@#_member_account` where `uid`='$sqluid'  ORDER BY `time` DESC");	
	...
	public function commissions(){
		...
		$invifriends=$mysql_model->GetList("select * from `@#_member` where `yaoqing`='$member[uid]' ORDER BY `time` DESC");
		  ...
		  for($i=0;$i
			   ...
			   //查询邀请好友给我反馈的佣金  
			   $recodes[$sqluid]=$mysql_model->
							GetList("select * from `@#_member_recodes` where `uid`='$sqluid' and `type`=1 ORDER BY `time` DESC");
	...
	public function cashout(){
       ...
       if(isset($_POST['submit1'])){ //提现	     
		 ...
		 $this->
									db->Query("INSERT INTO `@#_member_cashout`(`uid`,`money`,`username`,`bankname`,`branch`,`banknumber`,`linkphone`,`time`)VALUES
	   ...
	   if(isset($_POST['submit2'])){//充值
		 ...
		 if($cashouthdtotal
		  ...
		  //插入记录
		 $account=$this->
											db->Query("INSERT INTO `@#_member_account`(`uid`,`type`,`pay`,`content`,`money`,`time`)VALUES
