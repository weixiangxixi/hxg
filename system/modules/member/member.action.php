...
class member extends admin {
	...
	public function tuikuan(){
		...
		$total=$this->db->GetCount("SELECT COUNT(*) FROM `@#_user_money_img` WHERE 1"); 
		...
		$members=$this->db->GetPage("SELECT * FROM `@#_user_money_img` WHERE 1",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0)); 

		foreach ($members as $key => $val) {
			$uid = $val['uid'];
			$data = $this->db->GetOne("SELECT * FROM `@#_member` WHERE `uid` = '$uid'");
			$members[$key]['mobile'] = $data['mobile'];
			$members[$key]['username'] = $data['username'];
			$members[$key]['zh_money'] = $data['money'];
			$members[$key]['email'] = $data['email'];
		}

		include $this->tpl(ROUTE_M,'member.tuikuan');	
	}

	//确认退款
	public function sure_tuikuan(){
		$uid = $this->segment(4);
		$str = $this->db->GetOne("SELECT `money` FROM `@#_member` WHERE `uid` = '$uid'");
		$money = $str['money'];
		$time = time();
		$this->db->Autocommit_start();
		$data = $this->db->Query("UPDATE `@#_member` SET `money` = `money`-'$money' WHERE `uid` = '$uid'");
		$arr = $this->db->Query("UPDATE `@#_user_money_img` SET `money` = '$money',`status` = '1' WHERE `uid` = '$uid'");
		$crr = $this->db->Query("INSERT INTO `@#_member_account` (`uid`,`type`,`pay`,`content`,`money`,`time`)VALUES('$uid','-1','账户','(平台退款)','$money','$time')");
		
		if($data && $arr && $crr){
			$this->db->Autocommit_commit();
			_message("确认成功","/index.php/member/member/tuikuan/");
		}else{
			$this->db->Autocommit_rollback();
			_message("确认失败","/index.php/member/member/tuikuan/");
		}
	}
	...
	public function modify(){
		...
		if(isset($_POST['submit'])){
			...
			$order_auth=htmlspecialchars(trim($_POST['order_auth']));	

			$bind_phone = htmlspecialchars(trim($_POST['bind_phone']));

			$zhuan_status=htmlspecialchars(trim($_POST['zhuan_status']));

			$vip=htmlspecialchars(trim($_POST['vip']));	

			//$sql="UPDATE `@#_member` SET `username`='$username',`email`='$email',`mobile`='$mobile',`password`='$password',`money`='$money',`jingyan`='$jingyan',`score`='$score',`emailcode`='$emailcode',`mobilecode`='$mobilecode',`img`='$img',`groupid`='$membergroup',`qianming`='$qianming',`order_auth`='$order_auth',`bind_phone`='$bind_phone',`zhuan_status`='$zhuan_status',`vip`='$vip' WHERE `uid`='$uid'";
			$sql="UPDATE `@#_member` SET `username`='$username',`email`='$email',`mobile`='$mobile',`password`='$password',`jingyan`='$jingyan',`score`='$score',`emailcode`='$emailcode',`mobilecode`='$mobilecode',`img`='$img',`groupid`='$membergroup',`qianming`='$qianming',`order_auth`='$order_auth',`bind_phone`='$bind_phone',`zhuan_status`='$zhuan_status',`vip`='$vip' WHERE `uid`='$uid'";
			$time = time();
			//$sql1="INSERT INTO `@#_member_account` (uid,type,pay,content,money,time) value ('$uid','2','账户','通过线下充值','$money','$time')";
			$this->db->Query($sql1);
	...
	public function xxcz(){
		...
		if($user){
			if($type == 'id'){
				$data = $this->db->GetOne("SELECT * FROM `@#_member` WHERE `uid` = '$user'");
			}else if($type == 'nickname'){
				$data = $this->db->GetOne("SELECT * FROM `@#_member` WHERE `username` = '$user'");
			}else if($type == 'email'){
				$data = $this->db->GetOne("SELECT * FROM `@#_member` WHERE `email` = '$user'");
			}else if($type == 'mobile'){
				$data = $this->db->GetOne("SELECT * FROM `@#_member` WHERE `mobile` = '$user'");
			}

			if($this->segment(4) && $this->segment(5)){
				$this->db->Autocommit_start();	
				$time = time();
				$str = $this->db->Query("INSERT INTO `@#_member_account` (uid,type,pay,content,money,time) VALUES ('$user','1','账户','通过线下充值','$money','$time')");
				$brr = $this->db->Query("UPDATE `@#_member` SET `money` = `money`+'$money' WHERE `uid`='$user'");
				if($str && $brr && $user == '73379'){
					$this->db->Autocommit_commit();
					_message("充值成功","/index.php/member/member/xxcz/".$user);
				}else{
					$this->db->Autocommit_rollback();
					_message("充值失败","/index.php/member/member/xxcz/".$user);
				}
			}

			$arr = $this->db->GetList("SELECT * FROM `@#_member_account` WHERE `uid` = '$user' and `type` = '1' AND `content` = '通过线下充值' order by `time` desc");
		}

		include $this->tpl(ROUTE_M,'member_xxcz');
	}
...
HTML;
	...
	public function commreview(){
	 ...
	 $id=intval($this->segment(4));
		 ...
		 if($is==1){
		  ...
		  $time = $this->db->Getone("SELECT * FROM `@#_member_recodes` WHERE `uid`='$audsta[uid]' and `time`='$audsta[time]'");
	...
	public function recharge(){
		...
		if(isset($_GET['sososubmit'])){

			$posttime1=isset($_GET['posttime1'])?$_GET['posttime1']:'';

			$posttime2=isset($_GET['posttime2'])?$_GET['posttime2']:'';
			...
			$chongzhi=isset($_GET['chongzhi'])?$_GET['chongzhi']:'';
			...
			if(empty($chongzhi) || $chongzhi=='请选择充值来源') $content=" AND (`content`='充值' or `content`='使用佣金充值到参与账户' or `content`='使用余额宝充值到参与账户' or `content`='通过微信公众号充值' or `content`='通过网络充值')";

			

			$yonghu=isset($_GET['yonghu'])?$_GET['yonghu']:'';
			...
			$yonghuzhi=isset($_GET['yonghuzhi'])?$_GET['yonghuzhi']:'';
		...
		if(empty($wheres)){
			...
			$total=$this->db->GetCount("SELECT count(*) FROM `@#_member_account` WHERE (`content`='充值' or `content`='使用佣金充值到参与账户' or `content`='使用余额宝充值到参与账户' OR `content`='通过微信公众号充值' OR `content`='通过网络充值') AND `type`='1'"); 

			$summoeny=$this->db->GetOne("SELECT sum(money) sum_money FROM `@#_member_account` WHERE (`content`='充值' or `content`='使用佣金充值到参与账户' or `content`='使用余额宝充值到参与账户' or `content`='通过微信公众号充值' or `content`='通过网络充值') AND `type`='1'"); 
		...
		if(empty($wheres)){
			...
			$recharge=$this->db->GetPage("SELECT * FROM `@#_member_account` WHERE (`content`='充值' or `content`='使用佣金充值到参与账户' or `content`='使用余额宝充值到参与账户' OR `content`='通过微信公众号充值' OR `content`='通过网络充值') AND `type`='1' order by `time` desc",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0)); 
	...
	function cz_order(){
		...
		$total = $this->db->GetCount("SELECT * FROM `@#_member_account` WHERE type=2 "); 
		$recharge=$this->db->GetPage("SELECT * FROM `@#_member_account` WHERE type=2 order by `time` desc",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0)); 
		$members=array();
		...
		if(isset($_GET['p'])){$pagenum=$_GET['p'];}else{$pagenum=1;}	
		$page->config($total,$num,$pagenum,"0");
		include $this->tpl(ROUTE_M,'member.cz_order');	
	}
	//消费记录
	...
	public function pay_list(){
		...
		if(isset($_GET['sososubmit'])){
			...
			if(!empty($posttime1) && !empty($posttime2)){ //如果2个时间都不为空
				...
				$timed = time();
				// if ($posttime2 > $timed) {
				// 	_message("后一个时间不能大于现在时间");
				// }
				$times= "`@#_member_go_record`.time>='$posttime1' AND `@#_member_go_record`.time<='$posttime2'";

			}
			$qf_data = $this->db->GetList("SELECT m.moneycount FROM `@#_member_go_record` AS m LEFT JOIN `@#_member` AS n ON m.uid=n.uid LEFT JOIN `@#_shoplist` AS k ON m.shopid=k.id WHERE k.cateid = '170' AND n.auto_user='0' AND m.time>'$posttime1' AND m.time < '$posttime2'");
			$qf_money = 0;
			foreach ($qf_data as $key => $val) {
				$qf_money += $val['moneycount'];
			}
			//$qf_data = $this->db->GetList("SELECT m.id FROM `@#_member_go_record` AS m LEFT JOIN `@#_member` AS n ON m.uid = n.uid LEFT JOIN `@#_shoplist` AS k ON m.shopid = k.id WHERE k.cateid = '170' ADN n.auto_user = '0' AND m.time >= '$posttime1' AND m.time < '$posttime2'");
           // $qf_money = 0;
            //var_dump(count($qf_data));
            //foreach($qf_data as $key=>$val){
            	//$qf_money += ($val['zongrenshu']-1)*$val['yunjiage']);
                  //var_dump($qf_money);
           // }
			...
			if($yonghu=='用户id'){
				...
				if($yonghuzhi){
					...
					$uid=" AND `@#_member_go_record`.uid ='$yonghuzhi'";
			...
			if($yonghu=='用户名称'){
				...
				if($yonghuzhi){
					...
					if($user_uid){
						...
						$uid=" AND `@#_member_go_record`.uid ='$user_uid[uid]'";
		...
		if(empty($wheres)){
			$time = time()-86400;
			//$total=$this->db->GetCount("SELECT count(`@#_member_go_record`.*) FROM `@#_member_go_record`,`@#_member` WHERE `@#_member_go_record`.uid= `@#_member`.uid and `@#_member`.auto_user = 0 and `@#_member_go_record`.time>'$time'"); 
			$total = $this->db->GetCount("SELECT count(m.*) FROM `@#_member_go_record` AS m LEFT JOIN `@#_member` AS n ON m.uid=N.uid WHERE n.auto_user='0' AND m.time>'$time'");

			//$summoeny=$this->db->GetOne("SELECT sum(`@#_member_go_record`.moneycount) sum_money FROM `@#_member_go_record`,`@#_member` WHERE `@#_member_go_record`.uid= `@#_member`.uid and `@#_member`.auto_user = 0"); 
			$summoeny = $this->db->GetOne("SELECT sum(m.moneycount) sum_money FROM `@#_member_go_record` AS m LEFT JOIN `@#_member` AS n ON m.uid=n.uid WHERE n.auto_user='0' AND m.time>'$time'");

		}else{

			$total=$this->db->GetCount("SELECT count(`@#_member_go_record`.*) FROM `@#_member_go_record` ,`@#_member` WHERE  `@#_member_go_record`.uid= `@#_member`.uid and `@#_member`.auto_user = 0 and $wheres "); 

			$summoeny=$this->db->GetOne("SELECT sum(`@#_member_go_record`.moneycount) sum_money FROM `@#_member_go_record` ,`@#_member` WHERE  `@#_member_go_record`.uid= `@#_member`.uid and `@#_member`.auto_user = 0 and $wheres"); 
		}
		...
		if(empty($wheres)){
			$time = time()-86400;
			//$pay_list=$this->db->GetPage("SELECT `@#_member_go_record`.* FROM `@#_member_go_record` ,`@#_member` WHERE `@#_member_go_record`.uid= `@#_member`.uid and `@#_member`.auto_user = 0 order by time desc",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0)); 
			$pay_list = $this->db->GetPage("SELECT m.* FROM `@#_member_go_record` AS m LEFT JOIN `@#_member` AS n ON m.uid=N.uid WHERE n.auto_user='0' AND m.time > '$time' order by m.id desc",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));

		}else{

			$pay_list=$this->db->GetPage("SELECT `@#_member_go_record`.* FROM `@#_member_go_record` ,`@#_member` WHERE  `@#_member_go_record`.uid= `@#_member`.uid and `@#_member`.auto_user = 0 and $wheres order by time desc",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0)); 
