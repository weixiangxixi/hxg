...
class invite extends base {
   ...
   public function commissions_rs(){
    ...
    function cashout(){
        ...
        //绑定银行卡信息
        $bank_user = $this->db->GetList("SELECT * FROM `@#_bank_user` WHERE `uid` = '$uid' ORDER BY `id` DESC");
        ...
        if(isset($_POST['submit1'])){ //提现
            ...
            if($total<100){
                ...
                $this->db->Query("INSERT INTO `@#_member_cashout`(`uid`,`money`,`username`,`bankname`,`province`,`city`,`banknumber`,`linkphone`,`time`)VALUES
			('$uid','$money','$username','$bankname','$province','$city','$banknumber','$linkphone','$time')");
                _messagemobile("申请成功！请等待审核！",WEB_PATH.'/mobile/invite/cashout');
            }
        }

        if(isset($_POST['submit2'])){//充值
            //file_put_contents("/www/wwwroot/m.xx.com/sql_log/cashout.log",date("Y-m-d H:i:s", time())."-{$uid}-{$_SERVER['PHP_SELF']}:佣金余额{$total}:".json_encode($_REQUEST)."\n",FILE_APPEND);
            ...
            //file_put_contents("/www/wwwroot/m.xx.com/sql_log/cashout-success.log",date("Y-m-d H:i:s", time())."-{$uid}-{$_SERVER['PHP_SELF']}:佣金余额{$total}:".json_encode($_REQUEST)."\n",FILE_APPEND);
			...
			('$uid','-2','$content','$money','$time')");
                $hhh = $this->db->Query("INSERT INTO `@#_yongjin_flag` (`uid`,`unique_time`)VALUES('$uid','$time')");
                if($account && $mrecode && $recode && $hhh){
    ...
    function bankCardVerify(){
        $name = trim($_POST['name']);
        $bank = intval($_POST['bank']);
        $member=$this->userinfo;
        $arr = $this->db->GetOne("SELECT * FROM `@#_bank_user` WHERE `uid` = '$member[uid]' AND `username` = '$name' AND `banknumber` = '$bank'");
        if(!$arr){
            $data = $this->db->Query("INSERT INTO `@#_bank_user` (`uid`,`username`,`banknumber`)VALUES('$member[uid]','$name','$bank')");
            if($data){
                $str['success'] = true;
                $str['message'] = "银行卡读取成功";
            }else{
    ...
    //用户删除银行卡绑定信息
    function bank_user_del(){
        $id = $this->segment(4);
        $member = $member=$this->userinfo;
        $str = $this->db->GetOne("SELECT * FROM `@#_bank_user` WHERE `id` = '$id' AND `uid` = '$member[uid]'");
        if($str){
            $data = $this->db->Query("DELETE FROM `@#_bank_user` WHERE `id` = '$id'");
            if($data){
                _messagemobile("删除成功！");
            }else{
                _messagemobile("删除失败！");
	...
	public function singlelist(){
		...
		include templates("mobile/user","singlelist");
            ...
            public function mycode2(){
                $member=$this->userinfo;
                if(!$member){
                    header("Location:/index.php/mobile/user/login");
                }
                $this->qrcode();
                $webname=$this->_cfg['web_name'];
            ...
            public function qrcode(){
                ...
                //var_dump($host);
                if (!file_exists($file)) {
                    $data = file_get_contents("http://qr.topscan.com/api.php?bg=ffffff&fg=000000&el=l&w=220&m=10&text={$host}/index.php/mobile/mobile/init/".$uid."/");
                    //$data = file_get_contents("http://qr.topscan.com/api.php?bg=ffffff&fg=000000&el=l&w=220&m=10&text={$host}/index.php/mobile/mobile/init/");

                    file_put_contents($file, $data);
                    $dz = "qrcode/".$id.".jpg";
                    $x = file_get_contents("http://".$_SERVER['HTTP_HOST'].'/sc.php?object='.$dz.'&img=./statics/templates/yungou/images/mobile/'.$dz);


                } 
                //header("Location:/qrcode.php?id=".$id);           
            }
  			public function mycode3(){
                $member=$this->userinfo;
                if(!$member){
                    header("Location:/index.php/mobile/user/login");
                }
                 ...
                 */
              	...
              	$url = 'http://' . $_SERVER['HTTP_HOST']."/qrcode4.php?id=".$uid."&code=".$code;
				file_get_contents($url);
                include templates("mobile/invite","mycode3");
            }
            public function qrcode3(){
                ...
                //var_dump($host);
                if (!file_exists($file)) {
                    $data = file_get_contents("https://api.qrserver.com/v1/create-qr-code/?size=220x220&data={$host}/index.php/mobile/mobile/init/".$uid."/");
                    //$data = file_get_contents("http://qr.topscan.com/api.php?bg=ffffff&fg=000000&el=l&w=220&m=10&text={$host}/index.php/mobile/mobile/init/");
                    file_put_contents($file, $data);

                } 
                //header("Location:/qrcode2.php?id=".$id);           
            }
