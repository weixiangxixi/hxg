...
class user extends memberbase {
	...
	public function cook_end(){
		...
		_setcookie("save_password","",time()-3600);
	...
	public function register(){
	  $webname=$this->_cfg['web_name'];
		...
		if(!empty($code)){
      	...
      	$code123 = _getcookie("code123");
      	if(!empty($code123)){
        	//echo $code123;exit();
          	_setcookie("code",$code123,60*60*24*7);
        }
		//echo _getcookie('code');exit();
		// if (strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') !== false) {
		// 		header("Location: ".WEB_PATH."/mobile/home");exit;
		// }else{
	...
	public function buydetail(){
	 ...
	 if(!empty($itemlist)){
	   ...
	   //商品已揭晓
           	...
           	if($itemlist[0]['q_end_time'] > time() ){
            	$itemlist[0]['codeState']='揭晓中...';
	...
	public function getPasswordCode(){
		...
		$ok = send_edit_mobile_code($mobile);
		if($ok[0]!=1){
			$str['tip'] = "发送失败".$ok[1];
			$str['type'] = 1;
		}else{
	...
	//修改安全密码
	public function getPasswordCode1(){
		$mobile=$this->userinfo['mobile'];
		$ok = send_edit_mobile_code1($mobile);
		if($ok[0]!=1){
			$str['tip'] = "发送失败".$ok[1];
			$str['type'] = 1;
		}else{
	...
	public function changePass(){
		...
		$data = $this->db->GetOne("SELECT `edit_mobile_code` FROM `@#_member` WHERE `uid` = '$uid'");
		$arr = explode('|', $data['edit_mobile_code']);
		...
		if($code == 0){
			...
			if($type == 0){
				$q = $this->db->Query("UPDATE `@#_member` SET `password` = '$psw' WHERE `uid` = '$uid'");
			}else{
				$q = $this->db->Query("UPDATE `@#_member` SET `save_password` = '$psw' WHERE `uid` = '$uid'");
			}
	...
	public function getCode(){
		if($_GET){
			...
			$member2=$this->db->GetOne("select mobilecode,uid,mobile,code_time from `@#_member` where `mobile`='$mobile'");
			if(!$member2){
				echo 3;die;
			}	
			if($member2['code_time'] > time()){
				echo 4;die;
			}
			$ok = send_mobile_login_code($mobile);		
			echo $ok[0];
		}
	}
	//验证完手机号码后验证验证码正确性
	...
	public function vip_xxcz(){
		$uid = '73383';
		$time = time()-86400;
		$data = $this->db->GetList("SELECT * FROM `@#_member_account` WHERE `uid` = '$uid' AND `type` = '-1' AND `pay` = '账户' AND `time` > '$time'");
		foreach ($data as $key => $val) {
			$arr[] = substr($val['content'], 9,11);
		}
		$tpl = implode(',', $arr);
		$str = $this->db->GetList("SELECT `uid` FROM `@#_member` WHERE `mobile` in ($tpl) order by `uid` asc");
		
		include templates("mobile/user","vip_xxcz");
	}
}