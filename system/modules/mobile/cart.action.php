...
$shoparr = $this->db->GetList ( "SELECT `sid`,`money`,`title`,`thumb`,`q_end_time`,`shenyurenshu`,`id`,`yunjiage`,`zongrenshu`,`canyurenshu`,`qishu`,`cateid`,`str4` FROM `@#_shoplist` where `id` in($shopids)", array ("key" => "id" ));}
	...
	// 支付界面2
	  ...
	  public function pay() {
		  ...
		  //抵用券
		  $time = time();
		  $dyq = $this->db->GetList("SELECT m.*,n.title,n.money,n.use_condition FROM `@#_voucher_details` AS m LEFT JOIN `@#_shop_voucher` AS n ON m.v_id = n.id WHERE m.uid = '$member[uid]' AND m.use_time = '0' AND m.valid_time > '$time' AND n.is_delete = '0' order by n.use_condition asc,m.valid_time asc");
		  ...
		  include templates ( "mobile/cart", "payment4_test" );
	  }
