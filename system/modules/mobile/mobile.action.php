...
//            $list[$k]['member_go_record'] = $member_go_record;
    ...
    public function lottery_list(){
        ...
        $str = $this->db->GetOne("SELECT `q_end_time` FROM `@#_shoplist` WHERE `id` = '$key' LIMIT 1");
        $timed = time();
        $rs = $this->db->GetList("select id,sid,title,money,zongrenshu,canyurenshu,shenyurenshu,qishu,maxqishu,thumb,renqi,time,`order`,q_uid,q_user,q_user_code,q_end_time  from `@#_shoplist` where `q_end_time` !='' and `q_end_time` > '$str[q_end_time]' and `cateid` != '173' ORDER BY `q_end_time` ASC");
        foreach ($rs as $key => $val) {
            $rs[$key]['djs'] = $val['q_end_time'] - time();
        }
        if($rs){
            $data['code'] = 1;
        }else{
    ...
    public function lottery_going_shoplist(){
        $oid=intval($_GET['oid']);
        if(!$oid){
            $rs = $this->db->GetList("SELECT id,sid,title,money,zongrenshu,canyurenshu,shenyurenshu,qishu,maxqishu,thumb,renqi,time,`order`,q_uid,q_user,q_user_code,q_end_time  from `@#_shoplist` where `q_end_time` !='' AND `cateid` != '173' ORDER BY `q_end_time` DESC LIMIT 6");
            foreach ($rs as $key => $val) {
                $rs[$key]['times'] = $val['q_end_time'] - time();
                $xx[$key] = $val['q_end_time'];
            }
        }else{
            $str = $this->db->GetOne("SELECT `q_end_time` FROM `@#_shoplist` WHERE `id` = '$oid' LIMIT 1");
            $rs = $this->db->GetList("SELECT id,sid,title,money,zongrenshu,canyurenshu,shenyurenshu,qishu,maxqishu,thumb,renqi,time,`order`,q_uid,q_user,q_user_code,q_end_time  from `@#_shoplist` where `q_end_time` !='' and `q_end_time` > '$str[q_end_time]' AND `q_uid` is not NULL ORDER BY `q_end_time` DESC LIMIT 5");
            foreach ($rs as $key => $val) {
                $rs[$key]['times'] = $val['q_end_time'] - time();
                $xx[$key] = $val['q_end_time'];
            }
        }
        array_multisort($xx,SORT_ASC,$rs);
        if($rs){
            $data['error'] = 0;
            $data['length'] = count($rs);
            $data['listItems'] = $rs;
        }else{
            $data['error'] = 1;
        }
    ...
    public function lottery_huode_shop(){
        $oid=intval($_GET['oid']);
        if(!$oid){
            $data['error'] = 1;
        }else{
            $rs = $this->db->GetOne("select `q_uid` from `@#_shoplist` where `id` = '$oid'");

            $data['error'] = 0;
            $data['user'] = get_user_name($rs['q_uid']);
            $data['uid'] = $rs['q_uid'];
        }
    ...
    public function init()
    {
        ...
        //最新揭晓
        $timed = time();
        $shopqishu = $this->db->GetList("select * from `@#_shoplist` where `q_end_time` !='' and `q_end_time` - $timed < 60 and `cateid` != '173' ORDER BY `q_end_time` DESC LIMIT 13");
        //获取轮播
        ...
        */
		...
		if(!empty($code)){
          //echo $code;exit();
			//取出用户id
			_setcookie("code123",$code,60*60*24*7);
          	if(is_numeric($code)){   
                //是数字
              	_setcookie("code456",$code,60*60*24*7);
            }
		}

        $yjwz = _getcookie("yjwz");
        if(empty($yjwz)){
            _setcookie("yjwz","1",60*60*24);
        }
        include templates("mobile/index", "index6");
    }
    public function init_cs()
    {
        ...
        include templates("mobile/index", "index_cs");
    }
    public function init_cs2()
    {
        ...
        include templates("mobile/index", "index_cs2");
    }
    ...
    public function initajax1()
    {
        ...
        $star = ($p - 1) * $end;
        $select_w = "and `cateid` != '170' and `cateid` != '173' and `cateid` != '177'";
        if ($select == 10) {
        ...
        $shoplist['length'] = count($shoplist);

        $this_time = time();
        if (count($shoplist) > 1) {
            if ($shoplist['time'] > $this_time - 86400 * 3)
                $shoplist['t_new_goods'] = 1;
        }
    ...
    public function initajax_tmg()
    {
        ...
        $count = $this->db->GetCount("select count(*) from `@#_shoplist` where `q_uid` is null and `canyurenshu` < `zongrenshu` $select_w");
    ...
    {
        $sort_id = $this->segment(4);
        $data_cate = $this->db->GetOne("SELECT * FROM `@#_category` WHERE `cateid` = $sort_id");
        $webname = $this->_cfg['web_name'];
    ...
    public function glist1()
    {
        ...
        $title = "商品列表_" . _cfg("web_name");
        $key = "所有商品";
        include templates("mobile/index", "glist2");
    }
    public function fen()
    {
    ...
    {
        //$webname=$this->_cfg['web_name'];
        $txt = urldecode(htmlspecialchars($this->segment(4)));
    ...
    public function glistajax()
    {
        ...
        if(empty($fen1)){
            ...
            $brand = $this->db->GetList("select * from `@#_brand` where `cateid` = '173' order by `order` DESC");
            $daohang = '所有分类';
        }else if($fen1 == '2'){
            $brand = $this->db->GetList("select * from `@#_brand` where `cateid` != '173' order by `order` DESC");
            $daohang = '所有分类';
        } else {
            $brand = $this->db->GetOne("select * from `@#_brand` where `id`='$fen1' order by `order` DESC");
            $daohang = $this->db->GetOne("select * from `@#_category` where `brandid` = '$fen1' order by `order` DESC");
            $daohang = $daohang['name'];
        }
        ...
        if(empty($fen1)){
            $count = $this->db->GetList("select * from `@#_shoplist` where `q_uid` is null and `cateid` != '177' and  `canyurenshu` < `zongrenshu` $select_w");
        }else if ($fen1 == '1') {
            $count = $this->db->GetList("select * from `@#_shoplist` where `q_uid` is null and `cateid` = '173' and  `canyurenshu` < `zongrenshu` $select_w");

        }else if($fen1 == '2') {
            $count = $this->db->GetList("select * from `@#_shoplist` where `q_uid` is null and `cateid` != '177' and `cateid` != '173' and  `canyurenshu` < `zongrenshu` $select_w");
        } else {
            if($brand['cateid'] == '173'){
                $count = $this->db->GetList("select * from `@#_shoplist` where `q_uid` is null and `brandid`='$fen1' and  `canyurenshu` < `zongrenshu` $select_w");
            }else{
                $count = $this->db->GetList("select * from `@#_shoplist` where `q_uid` is null and `cateid`='$brand[cateid]' and  `canyurenshu` < `zongrenshu` $select_w");
            }
        }
        if(empty($fen1)){
            $shoplist = $this->db->GetList("select cateid,id,sid,title,money,zongrenshu,canyurenshu,shenyurenshu,qishu,maxqishu,thumb,renqi,time,`order` from `@#_shoplist` where `q_uid` is null and `cateid` != '177' and  `canyurenshu` < `zongrenshu` $select_w limit $star,$end");
        }else if ($fen1 == '1') {
            $shoplist = $this->db->GetList("select cateid,id,sid,title,money,zongrenshu,canyurenshu,shenyurenshu,qishu,maxqishu,thumb,renqi,time,`order` from `@#_shoplist` where `q_uid` is null and `cateid` = '173' and  `canyurenshu` < `zongrenshu` $select_w limit $star,$end");
        } else if($fen1 == '2'){
            $shoplist = $this->db->GetList("select cateid,id,sid,title,money,zongrenshu,canyurenshu,shenyurenshu,qishu,maxqishu,thumb,renqi,time,`order` from `@#_shoplist` where `q_uid` is null and `cateid` != '177' and `cateid` != '173' and  `canyurenshu` < `zongrenshu` $select_w limit $star,$end");

        } else {
            if($brand['cateid'] == '173'){
                $shoplist = $this->db->GetList("select cateid,brandid,id,sid,title,money,zongrenshu,canyurenshu,shenyurenshu,qishu,maxqishu,thumb,renqi,time,`order` from `@#_shoplist` where `q_uid` is null and `brandid`='$fen1' and  `canyurenshu` < `zongrenshu` $select_w limit $star,$end");
            }else{
                $shoplist = $this->db->GetList("select cateid,brandid,id,sid,title,money,zongrenshu,canyurenshu,shenyurenshu,qishu,maxqishu,thumb,renqi,time,`order` from `@#_shoplist` where `q_uid` is null and `cateid`='$brand[cateid]' and  `canyurenshu` < `zongrenshu` $select_w limit $star,$end");
            }
        }
    ...
    public function delCollect(){
        ...
        $this->db->Query("DELETE FROM `@#_collection` where `sid`= '$sid' AND `uid` = '$uid'");
    ...
    public function clistajax()
    {
        ...
        foreach ($shoplist as $k => $v) {
            $rs = $this->db->GetOne("select `id`,`thumb`,`qishu`,`title`,`canyurenshu`,`zongrenshu`,`shenyurenshu` from `@#_shoplist` where `sid`=".$v['sid']." order by time desc limit 1");
            if($rs){
    ...
    public function clistajax_ssc()
    {
        ...
        $shoplist = $this->db->GetList("select * from `@#_collection` where `uid` = $uid order by id");

        foreach ($shoplist as $k => $v) {
            $rs = $this->db->GetOne("select `id`,`thumb`,`qishu`,`title`,`canyurenshu`,`zongrenshu`,`shenyurenshu` from `@#_shoplist` where `cateid` = '173' and `sid`=".$v['sid']." order by time desc limit 1");
            if($rs){
                $list[$k] = array_merge($rs,array('cid'=>$v['id']));
            }
        }
...
//ajax获取商品列表信息
    ...
    public function item()
    {
        ...
        if($item['cateid'] == 170){
            ...
            $time2 = $time1+86400;
            $str_xxx = $this->db->GetList("SELECT * FROM `@#_shoplist` WHERE `sid` = '$sid' and `q_end_time` >= '$time1' and `q_end_time` < '$time2'");
            $i = 0;
            foreach ($str_xxx as $key => $val) {
                if($item['id'] == $val['id']){
                    $xxx = $i;
                    break;
                }
            ...
            $arr_xxx = $this->db->GetOne("SELECT * FROM `@#_bzqf_num` WHERE `id` = '1'");
          
            if(count($str_xxx) >= $arr_xxx['num'] && $i >= $arr_xxx['num']){
                 _messagemobile("今日该商品抢购已上限,明天再来哦！");
            }
        }
        ...
        if(!empty($itemlist)){
        //查询上期的获奖者信息
            $shangqi_qishu=$itemlist[0]['qishu'];
            $gorecode=$this->db->GetOne("select * from `@#_member_go_record` where `shopid`='".$itemlist[0]['id']."' AND `shopqishu`='".$itemlist[0]['qishu']."' and huode!=0 ORDER BY id DESC LIMIT 1");
        }
    ...
    public function dataserver()
    {
        ...
        $ssc = $this->db->GetOne("SELECT `ssc_number`,`end_time` FROM `@#_tmg` WHERE `shopid` = '$itemid' LIMIT 1");
        ...
        $item['ssc_end_time'] = $ssc['end_time'];
        $item['ssc_end_date'] = date('Y-m-d H:i:s',$ssc['end_time']+3600);
        if (empty($item['q_user_code']) && $item['cateid'] != '173') {
            _messagemobile("该商品正在进行中...");
        }

        //$itemlist = $this->db->GetList("select * from `@#_shoplist` where `sid`='$item[sid]' order by `qishu` DESC limit 250");

        $itemlist = $this->db->GetList("select * from `@#_shoplist` where `sid`='$item[sid]' and `q_end_time` is not null order by `time` DESC limit 200");
        ...
        $itemlist_x = $this->db->GetOne("select * from `@#_shoplist` where `sid`='$item[sid]' and `qishu` = '$item[qishu]'-1 order by time desc limit 1");
        $itemid2 = $itemlist_x['id'];

        $itemlist = $this->db->GetList("select * from `@#_shoplist` where `sid`='$item[sid]' and `time`>='$time_1' order by `qishu` DESC limit 200");
        ...
        $qishu_x = $itemlist_x['qishu'];

        $gorecode = $this->db->GetOne("select * from `@#_member_go_record` where `shopid`='" . $itemid2 . "' and huode!=0 LIMIT 1");
        $gorecode_count = $this->db->GetOne("select sum(gonumber) as count from `@#_member_go_record` where `shopid`='" . $itemid2 . "' AND `shopqishu`='" . $qishu_x . "' and `uid`= '$item[q_uid]'");
        $gorecode_count = $gorecode_count ? $gorecode_count['count'] : 0;

        $gorecode2 = $this->db->GetOne("select * from `@#_member_go_record` where `shopid`='" . $itemid . "' AND `shopqishu`='" . $item['qishu'] . "' and `uid`= '$item[q_uid]' and huode!=0 LIMIT 1");
        $gorecode_count2 = $this->db->GetOne("select sum(gonumber) as count from `@#_member_go_record` where `shopid`='" . $itemid . "' AND `shopqishu`='" . $item['qishu'] . "' and `uid`= '$item[q_uid]'");
        $gorecode_count2 = $gorecode_count2 ? $gorecode_count2['count'] : 0;
    ...
    public function ssclottery()
    {
        $webname = $this->_cfg['web_name'];
        $key = "透明购最新揭晓";
        include templates("mobile/index", "ssclottery");
    }

    // public function ssc(){
    //     $webname = $this->_cfg['web_name'];
    //     $key = "透明购";
    //     $data = $this->db->GetList("SELECT `id`,`name` FROM `@#_brand` WHERE `cateid` = '173' AND `order` = '1'");
    //     include templates("mobile/index", "ssc");
    // }
    ...
    public function buyrecords_test()
    {
        $webname = $this->_cfg['web_name'];
        $key = "所有参与记录";
        $itemid = intval($this->segment(4));
        $cords = $this->db->GetList("select * from `@#_member_go_record` where `shopid`='$itemid' order by `time` desc ");
        $co = count($cords);
        include templates("mobile/index", "buyrecords_test");
    }
    ...
    public function goodsdesc()
    {
        ...
        if (!$desc) {
      	...
      	preg_match_all('/(src=\"[:\.\/\w]+\")/', $desc['content'], $list);
        $list = $list[0];
        foreach ($list as $k => $v) {
            $desc['content'] = str_replace($v, 'src="http://f.weimicm.com'.trim(strchr($v, '/shopimg/'), '"').'@!water_image"', $desc['content']);
        }
        include templates("mobile/index", "goodsdesc");
    }
	public function goodsdesc1()
    {
        $webname = $this->_cfg['web_name'];
        $key = "图文详情";
        $itemid = intval($this->segment(4));
    ...
    public function calResult()
    {
        $itemid = intval($this->segment(4));
        $item = $this->db->GetOne("select * from `@#_shoplist` where `id`='$itemid' LIMIT 1");
        $ssc = $this->db->GetOne("SELECT `ssc_number` FROM `@#_tmg` WHERE `shopid` = '$itemid' LIMIT 1");
        ...
        $w = substr($item['q_end_time'], 11, 3);
        $user_shop_time_add = $h . $i . $s . $w;
        $total_number = fmod($user_shop_time_add * 100, $item['canyurenshu']);

        if ($item['q_content']) {
            $item['q_content'] = unserialize($item['q_content']);
            $user_shop_time_add = $item['q_counttime'];
            $total_number = fmod($item['q_counttime'], $item['canyurenshu']);
        }
    ...
    public function userindex()
    {
        ...
        if (!empty($member['headimg'])) {
            ...
            $member['img'] = OSS_UPLOAD_PATH . '/' . $member['img'] . '@!thumb_110_110';
        }
        //获取参与等级  参与新手  参与小将==
        ...
        if($member['auto_user'] == 1){
            if($member['score'] <= 100000){
                $member['yungoudj'] = "小将";
            }else if($member['score'] > 100000 && $member['score'] <= 500000){
                $member['yungoudj'] = "少将";
            }else if($member['score'] > 500000 && $member['score'] <= 2000000){
                $member['yungoudj'] = "中将";
            }else if($member['score'] > 2000000 && $member['score'] <= 5000000){
                $member['yungoudj'] = "上将";
            }else if($member['score'] > 5000000 && $member['score'] <= 10000000){
                $member['yungoudj'] = "大将";
            }else if($member['score'] > 10000000){
                $member['yungoudj'] = "将军";
            }
        }
    ...
    public function fenlei_ajax(){
        $data = $this->db->GetList("select `cateid`,`name`,`pic_url` from `@#_category` where `model`='1' and `parentid` = '0' and `cateid` != '170' order by `order` desc");
        foreach ($data as $key => $val) {
    ...
    public function upload_img(){
        ...
        $linkinfo=$this->db->GetOne("SELECT * FROM `@#_user_money_img` where `uid`='$user[uid]'");
        ...
        if(isset($_POST['submit'])){        
            ...
            if($linkinfo){
                $data = $this->db->Query("UPDATE `@#_user_money_img` SET `img` = '$logo' WHERE `uid` = '$user[uid]'");
   
            }else{
                $data = $this->db->Query("INSERT INTO `@#_user_money_img` (`uid`,`img`)VALUES('$user[uid]','$logo')");
            } 
            _messagemobile("提交成功");
        }
        include templates("mobile/test","upload_img"); 
    }

    public function guizessc(){
        include templates("mobile/index","guizessc");
    }

    public function game_tiyan(){
        include templates("tiyan/game","index");
    }
    ...
    public function get_fenlei(){
        $flag = $_POST['flag'];
        if($flag == 'tmg'){
            $data = $this->db->GetList("SELECT * FROM `@#_brand` WHERE `status`='Y' AND`cateid` = '173' AND `order` = '1' order by `id` asc");
        }else if($flag == 'pt'){
            $data = $this->db->GetList("SELECT n.id,m.cateid,m.name from `@#_category` as m left join `@#_brand` as n on m.cateid = n.cateid where m.model='1' and m.parentid = '0' and m.cateid not in('168','170','173','177') order by m.order desc");
        }
        echo json_encode($data);
    }
