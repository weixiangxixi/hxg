...
/* 		if(ROUTE_A!='userphotoup' and ROUTE_A!='singphotoup'){
	...
	public function getUserBuyList(){
	   ...
	   if($state==-1){
	     //参与参与的商品 全部...
		  ...
		  $shoplistall['listItems']=$this->db->GetList("select b.cateid,b.id,b.sid,b.title,b.money,b.zongrenshu,b.canyurenshu,b.shenyurenshu,b.qishu,b.maxqishu,b.thumb,b.renqi,b.time,b.`order`,b.q_uid,b.q_user,b.q_user_code,b.q_end_time,a.code,a.code_tmp,a.gonumber,a.goucode,a.huode,a.id,a.moneycount,a.pay_type,a.shopid,a.shopname,a.shopqishu,a.status,a.uid,a.uphoto,a.username,sum(gonumber) as gonumber from `@#_member_go_record` a left join `@#_shoplist` b on a.shopid=b.id where a.uid='$member[uid]' GROUP BY shopid order by a.time desc limit $FIdx,$EIdx " );
		}elseif($state==1){
		  //参与参与的商品 进行中...
		  $shoplist=$this->db->GetList("select *,sum(gonumber) as gonumber from `@#_member_go_record` a left join `@#_shoplist` b on a.shopid=b.id where a.uid='$member[uid]' and b.q_end_time is null and b.zongrenshu > b.canyurenshu GROUP BY shopid " );
		  //GROUP BY shopid 

		  $shoplistall['listItems']=$this->db->GetList("select b.cateid,b.id,b.sid,b.title,b.money,b.zongrenshu,b.canyurenshu,b.shenyurenshu,b.qishu,b.maxqishu,b.thumb,b.renqi,b.time,b.`order`,b.q_uid,b.q_user,b.q_user_code,b.q_end_time,a.code,a.code_tmp,a.gonumber,a.goucode,a.huode,a.id,a.moneycount,a.pay_type,a.shopid,a.shopname,a.shopqishu,a.status,a.uid,a.uphoto,a.username,sum(gonumber) as gonumber from `@#_member_go_record` a left join `@#_shoplist` b on a.shopid=b.id where a.uid='$member[uid]' and b.q_end_time is null and b.zongrenshu > b.canyurenshu GROUP BY shopid order by a.time desc limit $FIdx,$EIdx " );
		}else{
		  //参与参与的商品 已揭晓...
		  $shoplist=$this->db->GetList("select sum(gonumber) as gonumber from `@#_member_go_record` a left join `@#_shoplist` b on a.shopid=b.id where a.uid='$member[uid]' and b.q_end_time is not null GROUP BY shopid" );


		  $shoplistall['listItems']=$this->db->GetList("select b.cateid,b.id,b.sid,b.title,b.money,b.zongrenshu,b.canyurenshu,b.shenyurenshu,b.qishu,b.maxqishu,b.thumb,b.renqi,b.time,b.`order`,b.q_uid,b.q_user,b.q_user_code,b.q_end_time,a.code,a.code_tmp,a.gonumber,a.goucode,a.huode,a.id,a.moneycount,a.pay_type,a.shopid,a.shopname,a.shopqishu,a.status,a.uid,a.uphoto,a.username,sum(gonumber) as gonumber from `@#_member_go_record` a left join `@#_shoplist` b on a.shopid=b.id where a.uid='$member[uid]' and b.q_end_time is not null GROUP BY shopid order by a.time desc limit $FIdx,$EIdx " );
		}
		...
		if(!empty($shoplistall['listItems'])){
		   ...
		   foreach($shoplistall['listItems'] as $key=>$val){
			  ...
			  $shoplistall['listItems'][$key]['q_user']=$this->jx_con(get_user_name($val['q_uid']),$val['q_end_time']);
             
              if(strlen($val['q_end_time']) == '10'){
              		$val['q_end_time'] = $val['q_end_time'].".000";
              }

              $shoplistall['listItems'][$key]['q_end_time']=$this->jx_con(microt($val['q_end_time']),$val['q_end_time']);

              $shoplistall['listItems'][$key]['q_user_code']=$this->jx_con($val['q_user_code'],$val['q_end_time']);
	...
	public function getUserOrderList1(){
	   $FIdx=safe_replace($this->segment(4));
	   $EIdx=10;//safe_replace($this->segment(5));

	   $member=$this->userinfo;
	   $time=time();
	   $order=$this->db->GetList("select * from `@#_member_go_record` a left join `@#_shoplist` b on a.shopid=b.id where b.q_uid='$member[uid]' and (`huode`>'10000000' or `goucode`='00000000000') and ((b.q_end_time<=$time and b.zhigou=0) or b.zhigou=1) and a.`type`='1' " );
       
	   $orderlist['listItems']=$this->db->GetList("select *,a.id as orid  from `@#_member_go_record` a left join `@#_shoplist` b on a.shopid=b.id where a.uid='$member[uid]' and (`huode`>'10000000' or `goucode`='00000000000') and ((b.q_end_time<=$time and b.zhigou=0) or b.zhigou=1) and a.`type`='1' order by a.time desc limit $FIdx,$EIdx " );
	...
	public function getUserOrderList(){
	   ...
	   $time = time();
	   $member=$this->userinfo;
	   $order=$this->db->GetCount("select count(*) from `@#_member_go_record`  where uid='$member[uid]' " );

	   $orderlist['listItems']=$this->db->GetList("select b.id,b.cateid,b.sid,b.title,b.money,b.zongrenshu,b.canyurenshu,b.shenyurenshu,b.qishu,b.maxqishu,b.thumb,b.renqi,b.time,b.`order`,b.q_uid,b.q_user,b.q_user_code,b.q_end_time,b.str1,b.str2,a.code,a.code_tmp,a.gonumber,a.goucode,a.huode,a.id,a.moneycount,a.pay_type,a.shopid,a.shopname,a.shopqishu,a.status,a.uid,a.uphoto,a.username,a.time,a.company,a.company_code,a.company_money,a.dizhi_id,a.dizhi_time,a.id as orid from `@#_member_go_record` a left join `@#_shoplist` b on a.shopid=b.id where b.q_uid='$member[uid]' and a.huode!=0 and b.q_end_time <$time order by a.time desc limit $FIdx,$EIdx " );
	...
	//获得的商品(批量填写地址)
	public function getUserOrderList_pl(){
	   $FIdx=safe_replace($this->segment(4));
	   $EIdx=100;//safe_replace($this->segment(5));
	   $time = time();
	   $member=$this->userinfo;
	   $order=$this->db->GetCount("select count(*) from `@#_member_go_record`  where uid='$member[uid]' " );

	   $orderlist['listItems']=$this->db->GetList("select b.id,b.cateid,b.sid,b.title,b.money,b.zongrenshu,b.canyurenshu,b.shenyurenshu,b.qishu,b.maxqishu,b.thumb,b.renqi,b.time,b.`order`,b.q_uid,b.q_user,b.q_user_code,b.q_end_time,b.str1,b.str2,a.code,a.code_tmp,a.gonumber,a.goucode,a.huode,a.id,a.moneycount,a.pay_type,a.shopid,a.shopname,a.shopqishu,a.status,a.uid,a.uphoto,a.username,a.time,a.company,a.company_code,a.company_money,a.dizhi_id,a.dizhi_time,a.id as orid from `@#_member_go_record` a left join `@#_shoplist` b on a.shopid=b.id where b.q_uid='$member[uid]' and a.huode!=0 and a.dizhi_id = 0 and b.q_end_time <$time and b.str1 = 0 order by a.time desc limit $FIdx,$EIdx " );
	...
	//获得普通的商品
	public function getUserOrderList_p_pt(){
	    $FIdx=safe_replace($this->segment(4));
	    $EIdx=10;//safe_replace($this->segment(5));
	    $type = $_GET['type'];
	    ...
	    if($type == 'pt'){
	   	  	$order=$this->db->GetCount("SELECT count(m.*) from `@#_member_go_record` as m left join `@#_shoplist` as n ON m.shopid = n.id where m.uid='$member[uid]' and n.str1 = '0'" );

	   		$orderlist['listItems']=$this->db->GetList("SELECT b.id,b.cateid,b.sid,b.title,b.money,b.zongrenshu,b.canyurenshu,b.shenyurenshu,b.qishu,b.maxqishu,b.thumb,b.renqi,b.time,b.`order`,b.q_uid,b.q_user,b.q_user_code,b.q_end_time,b.str1,b.str2,a.code,a.code_tmp,a.gonumber,a.goucode,a.huode,a.id,a.moneycount,a.pay_type,a.shopid,a.shopname,a.shopqishu,a.status,a.uid,a.uphoto,a.username,a.time,a.company,a.company_code,a.company_money,a.dizhi_id,a.dizhi_time,a.id as orid from `@#_member_go_record` a left join `@#_shoplist` b on a.shopid=b.id where b.q_uid='$member[uid]' and a.huode!=0 and b.q_end_time <$time and b.str1 = 0 order by a.dizhi_id asc,a.time desc limit $FIdx,$EIdx " );
	    }else if($type == 'scq'){
	    	$order=$this->db->GetCount("SELECT count(m.*) from `@#_member_go_record` as m left join `@#_shoplist` as n ON m.shopid = n.id where m.uid='$member[uid]' and n.str1 = '0'" );

	   		$orderlist['listItems']=$this->db->GetList("SELECT b.id,b.cateid,b.sid,b.title,b.money,b.zongrenshu,b.canyurenshu,b.shenyurenshu,b.qishu,b.maxqishu,b.thumb,b.renqi,b.time,b.`order`,b.q_uid,b.q_user,b.q_user_code,b.q_end_time,b.str1,b.str2,a.code,a.code_tmp,a.gonumber,a.goucode,a.huode,a.id,a.moneycount,a.pay_type,a.shopid,a.shopname,a.shopqishu,a.status,a.uid,a.uphoto,a.username,a.time,a.company,a.company_code,a.company_money,a.dizhi_id,a.dizhi_time,a.id as orid from `@#_member_go_record` a left join `@#_shoplist` b on a.shopid=b.id where b.q_uid='$member[uid]' and a.huode!=0 and b.q_end_time <$time and b.str1 != 0 order by b.str2 asc,a.time desc limit $FIdx,$EIdx " );
	    }else{
	   		$order=$this->db->GetCount("SELECT count(*) from `@#_member_go_record`  where uid='$member[uid]' " );
	   ...
	   $EIdx=10;//safe_replace($this->segment(5));
	    ...
	    foreach ($Rechargelist as $key => $val) {
	    	if(strpos($val['content'], '不中全返') !== false && time() <= ($val['time'] + 30)){
		  		unset($Rechargelist[$key]);
		  	}
	    }

	    $str = $this->db->GetList("select * from `@#_member_account` where `uid`='$member[uid]' and `pay` = '账户' and `type`='1' ORDER BY time DESC limit $FIdx,$EIdx ");
        ...
        if(empty($Recharge['listItems'])){
		  ...
		  foreach($Recharge['listItems'] as $key=>$val){
		    ...
		    if(strpos($val['content'],"获得用户") !== false){
		    	$Recharge['listItems'][$key]['content'] = "线下充值".$val['money']."元";
		    }
		  }
		    $Recharge['code']=0;
		}
	...
	public function getUserConsumption(){
       ...
       foreach ($Consumptionlist as $key => $val) {
	    	if(strpos($val['content'], '不中全返') !== false && time() <= ($val['time'] + 30)){
		  		unset($Rechargelist[$key]);
		  	}
	    }
	   $str=$this->db->GetList("select * from `@#_member_account` where `uid`='$member[uid]' and `pay` in ('账户','抵用券') and `type`='-1'  ORDER BY time DESC limit $FIdx,$EIdx ");
       foreach ($str as $key => $val) {
	    	if(strpos($val['content'], '不中全返') !== false && time() <= ($val['time'] + 30)){
		  		unset($str[$key]);
		  	}
		  	if($val['pay'] == '抵用券'){
		  		$str[$key]['content'] = "抵用券购买了商品";
		  	}
	    }
      	$str = array_values($str);
      	$Consumption['listItems'] = $str;
