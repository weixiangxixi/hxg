	...
	public function order(){
		...
		if(!empty($_POST['phone'])){
			...
			if (strpos($phone,$mobile) !== false) {
				...
				foreach ($user as $k => $v) {
					$user_record[$k] = $this->db->GetList("select * from `@#_member_go_record` where huode!=0 and  `dizhi_time`>'$posttime1' and `dizhi_time`<'$posttime2' and  `dizhi_id` = ".$v['id']." order by dizhi_time desc ");
				}
