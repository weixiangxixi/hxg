    ...
    <tbody>
		<?php foreach($recordlist AS $v) {	?>		
            <tr>
                ...
                <td align="center">￥<?php $shop=$this->db->GetOne("SELECT * FROM `@#_shoplist` where id=".$v['shopid']); echo $shop['money']; ?>元</td>
                ...
                <td align="center"><?php $cid=$this->db->GetOne("SELECT * FROM `@#_dadan_setting` where sid=".$v['shopid']); if($cid['status']==1){echo '<input type="checkbox" checked="checked" value="'.$v['shopid'].'" onclick="haha(this);" />';}else{echo '<input type="checkbox" value="'.$v['shopid'].'" onclick="haha(this);" />';} ?></td>
...
<script>
    ...
    function post_ajax(id,s){
        $.ajax({  
            type:'post',    
            url:'/index.php/admin/dingdan/chang_dadan',  
            data:{
                id : id,  
                s : s
            }, 
