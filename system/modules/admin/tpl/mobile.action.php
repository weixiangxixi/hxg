...
//            $list[$k]['member_go_record'] = $member_go_record;
    ...
    public function initajax1()
    {
        ...
        */
        ...
        $end = 10;
        $star = ($p - 1) * $end;
        $select_w = "and `cateid` != '170' and `cateid` != '177'";
        if ($select == 10) {
...
//ajax获取商品列表信息
    ...
    public function dataserver()
    {
        ...
        $itemlist_x = $this->db->GetOne("select * from `@#_shoplist` where `sid`='$item[sid]' and `qishu` = '$item[qishu]'-1 limit 1");
        ...
        $gorecode = $this->db->GetOne("select * from `@#_member_go_record` where `shopid`='" . $itemid2 . "' AND `shopqishu`='" . $qishu_x . "'and huode!=0 LIMIT 1");
