...
<span style="margin-top: 30px;display: block">总金额：
    <?php 
        $k = 0;
        foreach($recordlist as $k=>$v) {
            ...
            if ($v['status']==1) {
                ...
                if($v['zhuijia'] != 0){
                    $zjer[$k] = intval($v['money']*($v['zhuijia']+1));
                }else{
  ...
  <table width="100%" cellspacing="0">
    ...
    <tbody>
		<?php foreach($recordlist AS $v) {	?>		
            <tr>
                <td align="center"><span style="font-size: 50px;color:red;"><?php $user = $this->db->GetOne("SELECT * FROM `@#_member` where uid=".$v['uid']); echo $v['bind_name']; ?></span></td>
...
<script>
    ...
    function post_ajax(id){
        ...
        //{ 
            $.ajax({  
                type:'post',    
                url:'/index.php/admin/dingdan2/check_wxpay',  
                data:{
                    id : id
                }, 
                dataType:'json',  
    ...
    function chehui_ajax(id){
        ...
        { 
            $.ajax({  
                type:'post',    
                url:'/index.php/admin/dingdan2/chehui_wxpay',  
                data:{
                    id : id
                }, 
                dataType:'json',  
    ...
    function zhui_ajax(id){
        ...
        { 
            $.ajax({  
                type:'post',    
                url:'/index.php/admin/dingdan2/add_wxpay',  
                data:{
                    id : id
                }, 
                dataType:'json',  
    ...
    $(function() {
        $("#post_data").click(function(){
            ...
            $.ajax({
                url:"/index.php/admin/dingdan2/song_money",
                method:"POST",
                dataType:"json",
                data:formData,
                success:function(data){
