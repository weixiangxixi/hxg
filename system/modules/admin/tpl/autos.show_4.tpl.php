...
<div class="header lr10" style="height:42px; line-height:42px;">
  ...
  3、指定触发页：域名/?/go/shuas/shua </font>
</div>
<div class="header lr10">
<input type="button" value="压力测试4(大)" style="float: left" class="button1">
<input type="button" value="返回" style="float: left" class="button1" onclick="window.location.href='/index.php/admin/test/auto'">
...
<div class="table_form lr10">
	...
	<tr>
		...
		<td><input  id="f_userid" type="text"  value="<?php echo $data['uf'];?>" class="input-text wid100" /> — <input  id="l_userid" type="text"  value="<?php echo $data['ul'];?>"  class="input-text wid100"/></td>
	...
	<td align="right" style="width:130px">指定购买商品：</td>&nbsp;&nbsp;&nbsp;&nbsp;<td><label id="shopid" style="color:red"><?php echo $shopid; ?></label></td>
...
<div class="header-data lr10" style="margin-top: 20px">
	<?php foreach ($arr as $key => $val) { ?>
		<input type ="submit" value="<?php echo $val['name']; ?>" name="zhye_submit" class="button"/>
   	<?php } ?>
</div>
<div class="table-list lr10" id="xxxx">
	 ...
	 <table width="100%" cellspacing="0">
	 	...
	 	<thead>
				...
				<tr>
					...
					<th width="30"><input id="cb_all" type="checkbox" class="cb_all" /></th>
		...
		<tbody>
			...
			<?php foreach($arr as $k=>$v){?>
				<?php $shoplist = $this->db->GetList("SELECT * FROM `@#_shoplist` WHERE (`q_uid` is null or `q_uid`=0) AND `cateid` = '$v[cateid]' order by `id` DESC "); ?>
...
$(function(){
	$("input[name='zhye_submit']").click(function(){
		var xx = $(this).index()+1;
		$('#xxxx table').css("display","none");
		$('#xxxx table:eq('+xx+')').css("display","");
	})

	// $("#xxxx table").each(function(){
	// 	var xx = true;
	// 	$(this).children("tbody").children().each(function(){
	// 		var x = $(this).children().eq(0).children(".cb").attr('checked');
	// 		if(!x){
	// 			xx = false;
	// 		}
	// 	})
	// 	if(xx == true){
	// 		$(this).children('thead').children().children().eq(0).attr('')
	// 	}
	// })
})
</script>
...
$(function(){
	...
	$("#start_auto_p").click(function(){
		...
		$.ajax({
			...
			url:"<?php echo WEB_PATH.'/admin/autos/ajaxaction4';?>",
	...
	$("#stop_auto_p").click(function(){
		...
		$.ajax({
			...
			url:"<?php echo WEB_PATH.'/admin/autos/stop4';?>",
	...
	$(".cb_all").click(function(){
		...
		if(checked =="checked"){
			if(index == 0){
				...
				$("#xxxx table:eq("+index+") .cb").attr("checked","checked");
			}else{
				$("#xxxx table:eq("+index+") .cb").attr("checked","checked");
			}
			...
			if(index == 0){
				...
				$("#xxxx table:eq("+index+") .cb").attr("checked",false);
			}
