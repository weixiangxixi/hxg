...
class fund1 extends admin {
	...
	public function specify(){
		if(isset($_POST['dosubmit'])){
			...
			$time = time();
			$appoint = $this->db->GetOne("SELECT * from `@#_appoint1` WHERE `shopid` ='$shopid' LIMIT 1");
			...
			$res = $this->db->Query("INSERT INTO `@#_appoint1` SET `shopid` = '$shopid',`userid` = '$userid',`time` = '$time'");
			//$res1 = $this->db->Query("UPDATE `@#_shoplist` SET `zhiding` = '$userid' WHERE `id`='$shopid'");
			if($res>0){
				_message("中奖人添加成功",G_ADMIN_PATH.'/fund1/specifylist/');
			}else{
				_message("中奖人添加失败");
			}
	...
	public function specifylist(){
		...
		$total=$this->db->GetCount("SELECT COUNT(*) FROM `@#_appoint1`  WHERE 1"); 
		...
		$res=$this->db->GetPage("SELECT * FROM `@#_appoint1` WHERE 1",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0)); 
	...
	public function zddel(){
		...
		$appoint = $this->db->GetOne("SELECT * from `@#_appoint1` WHERE `id` ='$id' LIMIT 1");
		//查询商品的sid
		$shopinfo = $this->db->GetOne("SELECT * from `@#_shoplist` WHERE `id` ='{$appoint['shopid']}' LIMIT 1");
		$res = $this->db->Query("DELETE FROM `@#_appoint1` WHERE (`id`='$id') LIMIT 1");
