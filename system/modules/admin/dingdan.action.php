...
class dingdan extends admin {
	...
	public function del_qrcode(){
		$id = intval($_POST['id']);
		$data = $this->db->Query("DELETE FROM `@#_wxpay_locat_config` WHERE `id` = '$id'");
		echo 1;
	}
	...
	public function get_dingdan(){
		...
		$zfb_data = $this->db->GetOne("SELECT sum(`money`) sum_money FROM `@#_alipay_locat` WHERE `uid`='$uid' AND `status` = '1' order by `create_time` desc");
		$zfb_money = $zfb_data['sum_money'];
		
		$zz_data = $this->db->GetOne("SELECT sum(`money`) sum_money FROM `@#_member_account` WHERE `uid`='$uid' and (`content` LIKE '%获得用户%' or `content`='使用佣金充值到参与账户') order by `time` desc");
		$zz_money = $zz_data['sum_money'];

		$gzh_data = $this->db->GetOne("SELECT sum(`money`) sum_money FROM `@#_member_account` WHERE `uid`='$uid' and `content`='通过微信公众号充值' order by `time` desc");
		$gzh_money = $gzh_data['sum_money'];

		$wl_data = $this->db->GetOne("SELECT sum(`money`) sum_money FROM `@#_member_account` WHERE `uid`='$uid' and `content`='通过网络充值' order by `time` desc");
		...
		//商城卡充值金额
		$sck_money = $this->db->GetOne("SELECT sum(`money`) sum_money FROM `@#_member_account` WHERE `uid` = '$uid' and `money` != '0' and `type` = '1' and `pay` = '账户' and `content` = '商城卡充值'");
	...
	public function mk_dingdan(){
		...
		$list = $this->db->GetList("select * from `@#_member_go_record` where `dizhi_id` != 0 and `status` LIKE  '%未发货%' and `company` = '新潮配送'");
		...
		$record=$this->db->GetPage("select * from `@#_member_go_record` where `dizhi_id` != 0 and `status` LIKE  '%未发货%' and `company` = '新潮配送' order by `time` ASC",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));	
	...
	public function mk_fahuo(){
		$id = $this->segment(4);
		$data = $this->db->GetOne("SELECT * FROM `@#_member_go_record` WHERE `dizhi_id` != 0 AND `status` LIKE '%未发货%' AND `company` = '新潮配送' AND `id` = '$id'");
		$time = time();
		if($data){
	...
	public function dh_mk_fahuo(){
		...
		if($data){
			$str = $this->db->Query("UPDATE `@#_exchange_record` SET `status` = '已发货',`company` = '新潮配送',`f_time` = '$time',`company_code` = '$company_code' WHERE `id` = '$id'");
			if($str){
				echo 1;
			}else{
				echo 2;
	...
	public function csg_mk_dingdan(){
		...
		$list = $this->db->GetList("select * from `@#_kh_record` where `dizhi_id` in($id) and `wuliu` =  '未发货' and `company` = '新潮配送'");
		...
		$record=$this->db->GetPage("select m.*,n.name from `@#_kh_record` AS m LEFT JOIN `@#_kh_shop` AS n ON m.shop_id = n.id where m.dizhi_id in($id) and m.wuliu =  '未发货' and m.company = '新潮配送' order by m.dizhi_time ASC",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));	
