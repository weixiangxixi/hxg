...
class autoc extends admin {	
	...
	public function show(){
		...
		$data  = $this->db->GetOne("SELECT `cfg_value` FROM `@#_wxch_cfg` WHERE  `cfg_name` = 'auto3'  LIMIT 1");
		...
		include $this->tpl(ROUTE_M,'autoc.show');
	...
	public function stop(){
			...
			$data['on'] = 0;
			$data = serialize($data);
			$rs = $this->db->Query("UPDATE `@#_wxch_cfg` SET `cfg_value` = '$data' WHERE `cfg_name` = 'auto3'");
			if($rs){
				echo "设置关闭成功";exit;
			}else{
				echo "设置关闭失败";exit;
	...
	public function ajaxaction(){
			...
			$temp["runtime"] = time();
			$temp = serialize($temp);
			$rs = $this->db->Query("UPDATE `@#_wxch_cfg` SET `cfg_value` = '$temp' WHERE `cfg_name` = 'auto3'");
			if($rs){
