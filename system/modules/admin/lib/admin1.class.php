...
class admin1 extends SystemAction
{
    protected $AdminInfo;
    private $db;
    ...
    public function __construct()
    {
        ...
        if (ROUTE_A != 'login' && ROUTE_A != 'index') {

            $CheckId = _encrypt(_getcookie("AID"), 'DECODE');
            $info = $this->db->GetOne("SELECT * FROM `@#_admin` WHERE `uid` = '$CheckId'");
            $data = explode(",", $info["mid"]);
            // if (!empty($info['mid'])) {
            //     $map = [];
            //     for ($i = 0; $i < count($data); $i++) {
            //         $item = $data[$i];
            //         $admin_action = $this->db->GetOne("SELECT * FROM `@#_admin_action` WHERE `action_id` = '$item'");
            //         $map[$admin_action["action_href"]] = true;
            //     }
            //     $href = "/" . ROUTE_C . "/" . ROUTE_A;
            //     $href_p = "/" . ROUTE_C;
            //     //var_dump($href);
            //     if (!$map[$href] && !$map[$href_p]) {
            //         //var_dump($href);
            //         //var_dump($href_p);
            //         exit("没有权限");
            //     }
