...
function admin_sck_details($str){
	$db = System::load_sys_class('model');
	$arr = $db->GetList("SELECT m.* FROM `@#_member_account` AS m LEFT JOIN `@#_member` AS n ON m.uid=n.uid WHERE `content` = '商城卡充值' AND n.auto_user='0' order by m.time desc");
	$money = 0; 
...
function admin_zhuan_details($uid,$time){
	$db = System::load_sys_class('model');
	$time1 = strtotime($time);
	$time2 = $time1 + 86400;
	$arr = $db->GetList("SELECT * FROM `@#_member_account` WHERE `uid` = '$uid' AND `time`>'$time1' AND `time`<'$time2' AND `type`='-1' AND `content` LIKE '%给用户%' order by `time` desc");
	return $arr;
}
