...
class dingdan2 extends admin {
	...
	public function bank(){
		...
		$total = 100;
		if (empty($wheres)) {
			$total = $this->db->GetCount("SELECT COUNT(*) FROM `@#_bank_locat`");
		}else{
			$total = $this->db->GetCount("SELECT COUNT(*) FROM `@#_bank_locat` where ".$wheres);
		}
		...
		if (empty($wheres)) {
			$recordlist=$this->db->GetPage("select * from `@#_bank_locat` order by id DESC ",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));
		}else{
			...
			$recordlist = $this->db->GetList("SELECT * FROM `@#_bank_locat` WHERE ".$wheres." order by id DESC");
		}
		...
		include $this->tpl(ROUTE_M,'dingdan.list55');	
	}
	...
	public function check_wxpay(){
		if (!empty($_POST)) {
			...
			$data = $this->db->GetOne("select * from `@#_bank_locat` where `id`=".$id);
			$uid = $data['uid'];
			$money = intval($data['money']);
			$content = "通过银行线下转账充值".$money."元";
			$this->db->Autocommit_start();
			$rs1 = $this->db->Query("UPDATE `@#_bank_locat` SET `status`='1',`update_time`=$timed,`aduser`=$CheckId where id='$id'");
	...
	public function add_wxpay(){
		if (!empty($_POST)) {
			...
			$rs1 = $this->db->Query("UPDATE `@#_bank_locat` SET `zhuijia`=`zhuijia` + '1' where id='$id'");
	...
	public function song_wxpay($id,$money){
		...
		$this->db->Autocommit_start();
		$rs1 = $this->db->Query("UPDATE `@#_bank_locat` SET `song`='$money' where id='$id'");
	...
	public function chehui_wxpay(){
		if (!empty($_POST)) {
			...
			$rs1 = $this->db->Query("UPDATE `@#_bank_locat` SET `status`='0',`update_time`=0,`aduser`=$CheckId where id='$id'");
