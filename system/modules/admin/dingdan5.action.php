...
class dingdan5 extends admin {
	...
	public function huafei(){
		...
		include $this->tpl(ROUTE_M,'dingdan.list5');	
	}

	public function huafei_status(){
		if (!empty($_POST)) {
			$id = intval($_POST['id']);
			$s = intval($_POST['s']);
	...
	public function check_wxpay(){
		if (!empty($_POST)) {
			...
			$rs1 = $this->db->Query("UPDATE `@#_wxpay_locat` SET `status`='1',`update_time`=$timed,`aduser`=$CheckId where id='$id'");
