...
//            $list[$k]['member_go_record'] = $member_go_record;
    ...
    public function lottery_list()
    {
        ...
        $timed = time();
        $rs = $this->db->GetOne("select * from `@#_shoplist` where `q_end_time` !='' and `q_end_time` - $timed < 60  ORDER BY `q_end_time` DESC LIMIT 1");
    ...
    public function glistajax()
    {
        ...
        if ($fen1) {
            $count = $this->db->GetList("select * from `@#_shoplist` where `is_hide` = 0 AND `q_uid` is null and `cateid`='$fen1' and  `canyurenshu` < `zongrenshu` $select_w");
        } else {
            $count = $this->db->GetList("select * from `@#_shoplist` where `is_hide` = 0 AND `q_uid` is null and  `canyurenshu` < `zongrenshu` $select_w");
        }
        if ($fen1) {
            $shoplist = $this->db->GetList("select * from `@#_shoplist` where `is_hide` = 0 AND `q_uid` is null and `cateid`='$fen1' and  `canyurenshu` < `zongrenshu` $select_w limit $star,$end");
        } else {
            $shoplist = $this->db->GetList("select * from `@#_shoplist` where `q_uid` is null and  `canyurenshu` < `zongrenshu` $select_w limit $star,$end");
        }
...
//ajax获取商品列表信息
    ...
    public function dataserver()
    {
        ...
        $key = "揭晓结果";
        $itemid = intval($this->segment(4));
        $item = $this->db->GetOne("select * from `@#_shoplist` where `is_hide` = '0' AND `id`='$itemid'  LIMIT 1");
        $sid = $item['sid'];
        if (!$item) {
