...
class dingdan1 extends admin {
	...
	public function alipay(){
		...
		if (empty($wheres)) {
			...
			$recordlist = $this->db->GetList("SELECT * FROM `@#_alipay_locat` WHERE ".$wheres." order by id DESC");
		}
	...
	public function edit_command(){
		...
		if(isset($_POST['submit'])){		
			...
			$sb = $this->db->Query("UPDATE `@#_wxpay_locat_config` SET `values`='$command' WHERE `id`='31'");

			if($sb){
				_message("修改成功",WEB_PATH.'/'.ROUTE_M.'/'.ROUTE_C."/alipay");
			}else{
				_message("修改失败");
			}		
		}
		include $this->tpl(ROUTE_M,'dingdan.edit_command');	
	}
	...
	public function song_wxpay($id,$money){
		...
		$rs1 = $this->db->Query("UPDATE `@#_alipay_locat` SET `song`='$money' where id='$id'");
	...
	public function alipay_sm(){
		...
		$total = 100;
		if (empty($wheres)) {
			$total = $this->db->GetCount("SELECT COUNT(*) FROM `@#_alipay_sm_details`");
		}else{
			$total = $this->db->GetCount("SELECT COUNT(*) FROM `@#_alipay_sm_details` where ".$wheres);
		}
		...
		if (empty($wheres)) {
			$recordlist=$this->db->GetPage("select * from `@#_alipay_sm_details` order by id DESC ",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));
		}else{
			$recordlist = $this->db->GetList("SELECT * FROM `@#_alipay_sm_details` WHERE ".$wheres." order by id DESC");
		}
		...
		include $this->tpl(ROUTE_M,'dingdan.list6');	
	}

	public function song_wxpay_sm($id,$money){
		$open = $this->db->GetOne("SELECT * FROM `@#_wxpay_locat_config` where `id`='18'");
		if ($open['open']==0) {
			return false;
		}
		...
		$data = $this->db->GetOne("select * from `@#_alipay_sm_details` where `id`=".$id);
		if ($data['status']==0) {
			echo 1;
			exit();
		}
		...
		$rs1 = $this->db->Query("UPDATE `@#_alipay_sm_details` SET `song`='$money' where id='$id'");
	...
	public function check_alipay_sm(){
		if (!empty($_POST)) {
			$id = intval($_POST['id']);
			$timed = time();
			...
			if(($timed - $data['create_time']) > 3600){
				echo 1;exit();
			}

			$rs1 = $this->db->Query("UPDATE `@#_alipay_sm_details` SET `status`='1',`update_time`=$timed,`aduser`=$CheckId where id='$id' AND `status` = '0'");

			$rs2 = $this->db->Query("UPDATE `@#_member` SET `money` = `money` + '$money' where (`uid` = '$uid')");
  			$rs3 = $this->db->Query("INSERT INTO `@#_member_account` (`uid`, `type`, `pay`, `content`, `money`, `time`) VALUES ('$uid', '1', '账户', '$content', '$money', '$timed')");

			if ($rs1 && $rs2 && $rs3) {
				$this->song_wxpay_sm($id,$money);
				echo 0;
			}else{
				echo 1;
			}
	...
	public function edit_qrcode_alipay(){
		$str = $this->db->GetList("SELECT distinct `fenzu` FROM `@#_alipay_sm` where `status`='1' order by `fenzu` asc");
      	$time_start = strtotime(date('Y-m-d',time()));
		$time_end = $time_start + 86400;
		$time_zt = $time_start - 86400;
      	foreach($str as $key=>$val){
        	$codelist[$val['fenzu']]['list'] = $this->db->GetList("SELECT * FROM `@#_alipay_sm` where `status`='1' and `fenzu` = '$val[fenzu]'");
          	
          	foreach ($codelist[$val['fenzu']]['list'] as $k => $v) {
              $aid = $v['id'];
              $data_today = $this->db->GetOne("SELECT sum(money) sum_money FROM `@#_alipay_sm_details` WHERE `aid` = '$aid' AND `status` = 1 AND `create_time` > '$time_start' AND `create_time` <= '$time_end'");
              $data_yest = $this->db->GetOne("SELECT sum(money) sum_money FROM `@#_alipay_sm_details` WHERE `aid` = '$aid' AND `status` = 1 AND `create_time` > '$time_zt' AND `create_time` <= '$time_start'");
              $codelist[$val['fenzu']]['list'][$k]['today'] = $data_today['sum_money'];
              $codelist[$val['fenzu']]['list'][$k]['yesterday'] = $data_yest['sum_money'];
          }
        }
		include $this->tpl(ROUTE_M,'dingdan.editqrcode2');
	}

	public function edit_qrcode_img_sm(){
		$id=intval($this->segment(4));
		$money=intval($this->segment(5));

		$img = "img_".$money;
		$linkinfo=$this->db->GetOne("SELECT `$img` FROM `@#_alipay_sm` where `id`='$id'");
		
		if(isset($_POST['submit'])){				
			$img_str = $_POST['img'];
			$this->db->Query("UPDATE `@#_alipay_sm` SET `$img`='$img_str' WHERE `id`='$id'");
			header("Location:/index.php/admin/dingdan1/edit_qrcode_img_sm/".$id."/".$money);	
		}
		include $this->tpl(ROUTE_M,'dingdan.wxpay_code2');	
	}
