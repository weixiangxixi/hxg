		...
		if(isset($_POST['dosubmit'])){	
			...
			foreach ($image as $key => $val) {
				...
				if($x == 'ok'){
					...
					file_get_contents("http://".$_SERVER['HTTP_HOST'].'/sc.php?object='.$val.'&img=./statics/uploads/'.$val);
					//echo '图片上传失败！';exit;
				}
...
public function goods_add_xg(){		
	...
	**/
	...
	public function shaidan_list(){
		...
		$data = $this->db->GetList("SELECT m.* FROM `@#_shoplist` AS m LEFT JOIN `@#_member` AS n ON m.q_uid = n.uid WHERE n.auto_user = '1' AND m.q_uid is not NULL AND `sid` = '$sid' order by `id` desc LIMIT 200");

		foreach ($data as $key => $val) {
			$id = $val['id'];
			$uid = $val['q_uid'];
			$str = $this->db->GetOne("SELECT * FROM `@#_shaidan` WHERE `sd_userid` = '$uid' AND `sd_shopid` = '$id'");
			if($str){
				$data[$key]['flag'] = 1;
			}else{
				$data[$key]['flag'] = 0;
	...
	public function shaidan_list_tj(){
		$id = $this->segment(4);
		$data = $this->db->GetOne("SELECT m.* FROM `@#_shoplist` AS m LEFT JOIN `@#_member` AS n ON m.q_uid = n.uid WHERE n.auto_user = '1' AND m.q_uid is not NULL AND `id` = '$id'");
		...
		if($_POST && $data){
			...
			$sd_time=time();
			$sd_ip = _get_ip();	
			$qishu= $this->db->GetOne("select `qishu`, `id`, `q_end_time` from `@#_shoplist` where `id`='$id'");
			$qs = $qishu['qishu'];
			$ids = $qishu['id'];
			...
			header("Location:".WEB_PATH."/admin/content/goods_shaidan_list");
		}
	
		include $this->tpl(ROUTE_M,'shop.shaidan_list_tj');
	}
