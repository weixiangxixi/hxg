...
class upload {
	...
	static public $uploadpath2 = null;
	...
	static public function upload_config($type=null,$size=null,$uploadpath=null){
		self::$uploadpath2 = $uploadpath."/".date("Ymd",time());
		if(empty($uploadpath)){
	...
	//上传
	static public function go_upload($file,$n){	
			...
			if(in_array($filetype,self::$uptype)){				
				if(is_uploaded_file(self::$files['tmp_name'])){	
					...
					if($error){
						$dz = self::$uploadpath2."/".self::$filename;
