...
final class mysql {
	...
	public function execute($sql) {
      	session_start();
        $tmp = explode("`go_shoplist`",$sql);
	    if(count($tmp) > 1){
            //file_put_contents("/www/wwwroot/m.xx.com/sql_log/sql_m.log","{$_SERVER['PHP_SELF']}:{$sql}\n",FILE_APPEND);
            $tt = explode("INTO",$tmp[0]);
            if(count($tt) > 1){
                //file_put_contents("/www/wwwroot/m.xx.com/sql_log/sql_m_in.log","{$_SERVER['PHP_SELF']}:{$sql}\n",FILE_APPEND);

            }
            $tt = explode("PDATE",$tmp[0]);
            if(count($tt) > 1){
                //file_put_contents("/www/wwwroot/m.xx.com/sql_log/sql_m_update.log","{$_SERVER['HTTP_HOST']}-{$_SERVER['PHP_SELF']}:{$sql}\n",FILE_APPEND);

            }
	    }
//	    file_put_contents("/www/wwwroot/m.xx.com/sql.log",$sql,FILE_APPEND);
      	
      	$uuuid = $_SESSION['uuuid'];
	    if(($uuuid == '94928' || $uuuid == '94521' || $uuuid == '101535' || $uuuid == '101532' || $uuuid == '101534' || $uuuid == '101529') && (strpos(strtoupper($sql), 'SELECT') === false && strpos($sql, 'shopcodes') === false)){
		    file_put_contents("/www/wwwroot/csthsc/sql_log/pay_call.log","{$_SERVER['PHP_SELF']}:{$sql}\n",FILE_APPEND);
		}
