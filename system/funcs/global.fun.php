...
//判断订单是否为购物（区别充值和购物）
function _isShopping($bill_no){//是充值还是购物
	$db=System::load_sys_class('model');
	$record = $db->GetOne("SELECT scookies  from `@#_member_addmoney_record` where `code` = '$bill_no'");
	if($record) 
	  if($record['scookies']) return true;
	  else return false;
	else return false;
}	
...
function _checkmobile($mobilephone=''){
	...
	if(preg_match("/^13[0-9]{1}[0-9]{8}$|17[0-9]{1}[0-9]{8}$|15[0-9]{1}[0-9]{8}$|14[0-9]{1}[0-9]{8}$|18[0-9]{1}[0-9]{8}$|16[0-9]{1}[0-9]{8}$|19[8-9]{1}[0-9]{8}$/",$mobilephone)){   
...
function _mobile_time_send($mobile,$content){
...
function _message($string=null,$defurl=null,$time=2,$config=null){
	...
	if(defined("G_IN_ADMIN")){
		...
		if(empty($config)){
			...
			$config = array("titlebg"=>"#DB3752","title"=>"#fff");
...
function _freshen(){
	...
	$shoplist=$db->GetList("select * from `@#_shoplist` where `canyurenshu` >= `zongrenshu`  and `q_end_time` is null order by time desc limit 0,188");
	if($shoplist){
		foreach($shoplist as $key =>$ginfo){
		  $jinri_time = time();		
		  if($ginfo['xsjx_time']!='0')   break;//_message("限时揭晓商品不能手动揭晓");
		  if($ginfo['shenyurenshu']!='0') break;//_message("该商品还有剩余人数,不能手动0揭晓！");
...
function JSON($array) {
   ...
   function arrayRecursive(&$array, $function, $apply_to_keys_also = false)
      ...
      curl_close($ch);
       ...
       //支付送流量
       function flow($total_fee_t,$uid){
       		$db=System::load_sys_class('model');
       	    $c_flow = $total_fee_t * 0.1;
	        $c_if = $db->GetOne("select * from `@#_member_flow` where `uid` = " . $uid);
	        if (empty($c_if)) {
	            $db->Query("INSERT INTO `@#_member_flow` (`uid`, `flow`) VALUES ('$uid', '$c_flow')");
	        } else {
	            $db->Query("UPDATE `@#_member_flow` SET `flow` = `flow` + $c_flow where (`uid` = '$uid')");
	        }
	        return true;
       }
       //新注册用户首次充值送8.88元
       function news_user_song($uid){
       	  $db=System::load_sys_class('model');
       	  $time = time();
       	  $new=$db->GetOne("select * from `@#_new_user` where `uid` = ".$uid);
		  if (!empty($new)) {
		  	 if($new['status'] == 1){
		  	 	$db->Query("UPDATE `@#_new_user` SET `status` = 0 where (`uid` = '$uid')");
		  	 	$db->Query("UPDATE `@#_member` SET `money` = `money` + '8' where (`uid` = '$uid')");
        		$db->Query("INSERT INTO `@#_member_account` (`uid`, `type`, `pay`, `content`, `money`, `time`) VALUES ('$uid', '1', '账户', '新注册用户首次充值送8元', '8', '$time')");
		  	 }	 	  
		  }
		  return true;
       }
       ...
       function cate_num($cate_id){
       		$db = System::load_sys_class('model');
       		$total = $db->GetCount("SELECT COUNT(*) FROM `@#_shoplist` WHERE `cateid`='$cate_id' and `q_uid` is null");
       		return $total;
       }
